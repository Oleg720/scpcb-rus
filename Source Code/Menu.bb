...
Function UpdateMainMenu()
	...
	Else
		...
		Select MainMenuTab
			Case 1 ; New game
				...
				IntroEnabled = DrawTick(x + 230 * MenuScale, y + 110 * MenuScale, IntroEnabled)	;280
				...
				Else
					...
					For i% = (1+(6*CurrLoadGamePage)) To 6+(6*CurrLoadGamePage)
						If i <= SaveGameAmount Then
							...
							If SaveGameVersion(i - 1) <> CompatibleNumber Then ; And SaveGameVersion(i - 1) <> "1.3.10"
								Color 255,0,0
							Else
								Color 255,255,255
							EndIf
			...
			Case 3,5,6,7 ;options
				...
				ElseIf MainMenuTab = 5 ;Audio
					...
					Color 255,255,255
					AAText x + 20 * MenuScale, y, "Автовысвобождение звука:" ;Sound auto-release:
				...
				ElseIf MainMenuTab = 6 ;Controls
					...
					Color(255, 255, 255)
					AAText(x + 20 * MenuScale, y, "Сглаживание мыши:") ;Mouse smoothing:
					If MouseOn(x+310*MenuScale,y-4*MenuScale,150*MenuScale+14,20)
					...
					AAText(x + 320 * MenuScale, y + 20 * MenuScale, "Моргнуть") ;280 ;Manual Blink
					InputBox(x + 430 * MenuScale, y + 20 * MenuScale,120*MenuScale,20*MenuScale,KeyName(Min(KEY_BLINK,210)),7) ;100
					AAText(x + 320 * MenuScale, y + 40 * MenuScale, "Бежать") ;280 ;Sprint
					InputBox(x + 430 * MenuScale, y + 40 * MenuScale,120*MenuScale,20*MenuScale,KeyName(Min(KEY_SPRINT,210)),8) ;100
					AAText(x + 320 * MenuScale, y + 60 * MenuScale, "Предметы") ;280 ;Open/Close Inventory
					InputBox(x + 430 * MenuScale, y + 60 * MenuScale,120*MenuScale,20*MenuScale,KeyName(Min(KEY_INV,210)),9);100
					AAText(x + 320 * MenuScale, y + 80 * MenuScale, "Ползти") ;280 ;Crouch
					InputBox(x + 430 * MenuScale, y + 80 * MenuScale,120*MenuScale,20*MenuScale,KeyName(Min(KEY_CROUCH,210)),10) ;100
					AAText(x + 320 * MenuScale, y + 100 * MenuScale, "Консоль") ;280 ;Open/Close Console
					InputBox(x + 430 * MenuScale, y + 100 * MenuScale,120*MenuScale,20*MenuScale,KeyName(Min(KEY_CONSOLE,210)),12) ;100
			...
			Case 4 ; load map
				...
				If SavedMaps(0)="" Then 
					AAText (x + 20 * MenuScale, y + 20 * MenuScale, "Нет сохранённых карт. Для создания новых используйте Редактор карт.") ;No saved maps. Use the Map Creator to create new maps.
				Else
					x = x + 20 * MenuScale
					y = y + 20 * MenuScale
					For i = (1+(6*CurrLoadGamePage)) To 6+(6*CurrLoadGamePage)
	...
	Color 255,255,255
	AASetFont BlitzFont ;ConsoleFont
			...
			If percent = 0 Then 
				...
				Else
					Select Rand(13)
						...
						Case 11
							SelectedLoadingScreen\txt[0] = "Сделайте её членом полуночной команды." ;Make her a member of the midnight crew.
						Case 12
...
Function DrawOptionsTooltip(x%,y%,width%,height%,option$,value#=0,ingame%=False)
	...
	Select lower(option$)
		...
		Case "particleamount"
			...
			End Select
		Case "vram"
			txt = "Текстуры, которые хранятся в видеопамяти (VRAM), будут загружаться быстрее. Однако это может вызвать ухудшение их качества." ;Textures that are stored in the Video-RAM will load faster, but this also has negative effects on the texture quality as well.
			txt2 = "Эта опция не может быть изменена во время игры." ;This option cannot be changed in-game.
			R = 255
			;[End Block]
		;Sound options
		...
		Case "usertrack"
			...
			txt2 = "Эта опция не может быть изменена во время игры." ;This option cannot be changed in-game.
		Case "usertrackmode"
			txt = "Порядок проигрывания пользовательский треков. "+Chr(34)+"Повтор"+Chr(34)+" - файлы воспроизводятся в алфавитном порядке. "+Chr(34)+"Случайно"+Chr(34)+" - файлы " ;Sets the playing mode for the custom tracks. "+Chr(34)+"Repeat"+Chr(34)+" plays every file in alphabetical order. "+Chr(34)+"Random"+Chr(34)+" chooses the 
			txt = txt + "воспроизводятся в случайном порядке." ;next track at random.
			R = 255
			G = 255
			txt2 = "Обратите внимание, что при воспроизведении в случайном порядке, треки могут повторяться." ;Note that the random mode does not prevent previously played tracks from repeating.
		Case "usertrackscan"
			txt = "Перепроверить папку на наличие новых или удалённых треков." ;Re-checks the user tracks directory for any new or removed sound files.
			;[End Block]
		;Control options	
			;[Block]
...
Function DrawMapCreatorTooltip(x%,y%,width%,height%,mapname$)
	...
	Else
		...
		descr$ = "[Отсутствует]" ;[No description]
		ramount% = 0
		hasForest% = False
		hasMT% = False
	EndIf
	...
	If ramount > 0 Then
		txt[3] = "Количество комнат: "+ramount ;Room amount: 
	Else
		txt[3] = "Количество комнат: [Неизвестно]" ;Room amount: [Unknown]
	EndIf
	If hasForest Then
