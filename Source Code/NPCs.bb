											...
											If KillTimer >= 0 And (Not GodMode) Then
												...
												Select PlayerRoom\RoomTemplate\Name
													...
													Case "173"
														...
														DeathMSG = DeathMSG + "во время нарушения, субъект был убит SCP-173, как только начались сбои в электрической сети." ;chamber during the breach, the subject was killed by SCP-173 as soon as the disruptions in the electrical network started.
													Case "room2doors"
														DeathMSG = Chr(34)+"Если я не ошибаюсь, одна из основных целей этих комнат - остановить SCP-173, чтобы он не продвигался далее во время нарушений условий содержания. " ;If I'm not mistaken, one of the main purposes of these rooms was to stop SCP-173 from moving further in the event of a containment breach. 
														DeathMSG = DeathMSG + "Так чьей же блестящей идеей было сделать там ЧЁРТОВУ ВЕНТИЛЯЦИЮ ЧЕЛОВЕЧЕСКИХ РАЗМЕРОВ?!"+Chr(34) ;So, who's brilliant idea was it to put A GODDAMN MAN-SIZED VENTILATION DUCT in there?
													Default 
														DeathMSG = "Субъект D-9341. Причина смерти: Смертельный перелом шеи. Предположительно, убит SCP-173." ;Subject D-9341. Cause of death: Fatal cervical fracture. Assumed to be attacked by SCP-173.
												End Select
									...
									If KillTimer >= 0 And FallTimer >= 0 Then
										...
										If Ceil(n\Frame) = 110 And (Not GodMode) Then
											...
											If PlayerRoom\RoomTemplate\Name = "pocketdimension" Then
												DeathMSG = "Субъект D-9341. Тело частично разложилось, предположительно из-за эффекта "+Chr(34)+"коррозии"+Chr(34)+" SCP-106. Тело утилизировано путём сжигания." ;Subject D-9341. Body partially decomposed by what is assumed to be SCP-106's "+Chr(34)+"corrosion"+Chr(34)+" effect. Body disposed of via incineration.
												Kill()
											Else
									...
									Else
										...
										If n\Target=Null Then
											If (Not GodMode) Then 
												...
												CameraShake = 30
												BlurTimer = 2000
												DeathMSG = "Большое количество крови найдено в [ДАННЫЕ УДАЛЕНЫ]. ДНК принадлежит субъекту D-9341. Скорее всего, был [ДАННЫЕ УДАЛЕНЫ] SCP-096." ;A large amount of blood found in [DATA REDACTED]. DNA indentified as Subject D-9341. Most likely [DATA REDACTED] by SCP-096.
												Kill()
												KillAnim = 1
												For i = 0 To 6
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									...
									If dist < 0.5 Then
										...
										Else
											...
											If (Not GodMode) Then
												If PlayerRoom\RoomTemplate\Name$ = "room049"
													DeathMSG = "Три (3) активных образца SCP-049-2 были найдены в туннеле рядом с камерой SCP-049. Уничтожены Девятихвостой Лисой." ;Three (3) active instances of SCP-049-2 discovered in the tunnel outside SCP-049's containment chamber. Terminated by Nine-Tailed Fox.
													For e.events = Each Events
														If e\EventName = "room049" Then e\EventState=-1 : Exit
													Next
												Else
													DeathMSG = "Активный образец SCP-049-2 был найден в [УДАЛЕНО]. Уничтожен Девятихвостой Лисой." ;An active instance of SCP-049-2 was discovered in [REDACTED]. Terminated by Nine-Tailed Fox.
													Kill()
												EndIf
									...
									If prevFrame < 23 And n\Frame=>23 Then
										If EntityDistance(n\Collider,Collider)<1.1
											If (Abs(DeltaYaw(n\Collider,Collider))<=60.0)
												...
												DeathMSG = "Субъект D-9341. Причина смерти: множественные раны и тяжёлые силовые травмы, вызванные SCP-049-2." ;Subject D-9341. Cause of death: multiple lacerations and severe blunt force trauma caused by an instance of SCP-049-2.
											EndIf
										EndIf
								...
								If n\Reload = 0 ;And n\Frame>1550 Then
									...
									If PickedEntity() = Collider Or n\State3=1 Then
										...
										If PlayerRoom\RoomTemplate\Name = "start" Then 
											DeathMSG = "Субъект D-9341. Причина смерти: Огнестрельное ранение в голову. Записи с камер наблюдения подтверждают, что субъект был убит агентом Алгрином вскоре после начала блокировки Зоны." ;Subject D-9341. Cause of death: Gunshot wound to the head. The surveillance tapes confirm that the subject was terminated by Agent Ulgrin shortly after the site lockdown was initiated.
											instaKillPlayer = True
										ElseIf PlayerRoom\RoomTemplate\Name = "exit1" Then
											DeathMSG = Chr(34)+"Агент G. Центру. Уничтожил убегающего сотрудника класса D во внутреннем дворе Выхода B."+Chr(34) ;Agent G. to control. Eliminated a Class D escapee in Gate B's courtyard.
										Else
											DeathMSG = ""
										EndIf
											...
											Else
												...
												If Injuries > 3.0 Then
													If PlayerRoom\RoomTemplate\Name = "room2offices" Then ;..! Надо будет удостовериться в качестве перевода. Ай да ладно, сделаю это позже.
														...
														DeathMSG = DeathMSG + "эта тентакля избила Класса D до смерти."+Chr(34) ;beaten some unfortunate Class D to death at some point during the breach.
													Else
														DeathMSG = Chr(34)+"Мне нужно что-то больше, чем обычная команда по уборке, чтобы позабоиться об этом. " ;We will need more than the regular cleaning team to take care of this. 
														DeathMSG = DeathMSG + "Две больших и очень активных тентакли, похоже, сформаровались " ;Two large and highly active tentacle-like appendages seem 
														...
														DeathMSG = DeathMSG + "что мы видели раньше. Похоже, что в какой-то момент " ;unlike anything we've seen before - it looks like they have 
														DeathMSG = DeathMSG + "эта тентакля избила Класса D до смерти."+Chr(34) ;beaten some unfortunate Class D to death at some point during the breach.
													EndIf
													Kill()
												EndIf
									...
									If ForestNPCData[2]=0
										...
										For i = 0 To MaxItemAmount-1
											If Inventory(i)<>Null
												...
												If docname = "Журнал #1" Or docname = "Журнал #2" Or docname = "Журнал #3" ;Log #
													;860,850,830,800
							...
							If n\LastSeen > 0 And (Not NoTarget) Then
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If Injuries>4.0 Then 
										DeathMSG=Chr(34)+"Все четверо (4) сбежавших экземпляров SCP-939 были захвачены и успешно задержаны. " ;All four (4) escaped SCP-939 specimens have been captured and recontained successfully. 
										DeathMSG=DeathMSG+"Трое (3) из них натворили настоящий погром в районе хранения 6. Была послана бригада чистильщиков."+Chr(34) ;Three (3) of them made quite a mess at Storage Area 6. A cleaning team has been dispatched.
										Kill()
				...
				If (dist<HideDistance) Then
					...
					If (WearingNightVision=0) Then
						...
						If dist<1 And n\Reload <= 0 And MsgTimer <= 0 Then
							Select Rand(6)
								Case 1
									Msg="Вы чувствуете, как что-то дышит рядом с Вами." ;You feel something breathing right next to you.
								Case 2
									Msg=Chr(34)+"Такое ощущение, как будто что-то рядом со мной в этой комнате."+Chr(34) ;It feels like something's in this room with me.
								Case 3
									Msg="Вы чувствуете, как будто что-то находится здесь, но Вы ничего не видите." ;You feel like something is here with you, but you do not see anything.
								Case 4
									Msg=Chr(34)+"Шалит ли со мной моя психика или кто-то ещё здесь?"+Chr(34) ;Is my mind playing tricks on me or is there someone else here?
								Case 5
									Msg="YВы чувствуете, как будто что-то преследует Вас." ;You feel like something is following you.
								Case 6
									Msg="Вы чувствуете что-то рядом с Вами, но не способны увидеть это. Возможно, время этого пришло." ;You can feel something near you, but you are unable to see it. Perhaps its time is now.
							End Select
                            n\Reload = 20*70
							MsgTimer=8*70
						EndIf
									...
									Else
										...
										If Injuries > 10.0
											...
											If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
												DeathMSG = "Всему персоналу, располагавшемуся в эвакуационном убежище LC-2 во время катастрофы, были введены " ;All personnel situated within Evacuation Shelter LC-2 during the breach have been administered 
												DeathMSG = DeathMSG + "амнезиаки класса B в связи с Инцидентом 1499-E. Субъект класса D, участвовавший в этом происшествии, " ;Class-B amnestics due to Incident 1499-E. The Class D subject involved in the event 
												DeathMSG = DeathMSG + "вскоре умер, будучи застреленным агентом [УДАЛЕНО]." ;died shortly after being shot by Agent [REDACTED].
											Else
												DeathMSG = "Неопознанный и мёртвый субъект D-Класса мужского пола был обнаружен в [УДАЛЕНО] Девятихвостой Лисой. " ;An unidentified male and a deceased Class D subject were discovered in [REDACTED] by the Nine-Tailed Fox. 
												DeathMSG = DeathMSG + "Человек, как было отмечено, был весьма возбуждён и, похоже, говорил только по-русски. " ;The man was described as highly agitated and seemed to only speak Russian. 
												DeathMSG = DeathMSG + "Он был отправлен во временную зону сдерживания в [УДАЛЕНО] до прибытия переводчика." ;He's been taken into a temporary holding area at [REDACTED] while waiting for a translator to arrive.
											EndIf
										EndIf
									EndIf
								ElseIf n\Frame => 99
						...
						If n\Reload =< 0 And KillTimer = 0 Then
							If EntityVisible(n\Collider, Camera) Then
								...
								If angle < 5 Or angle > 355 Then 
									...
									DeathMSG="Субъект D-9341. Погиб от потери крови после выстрела Девятихвостой Лисы." ;Subject D-9341. Died of blood loss after being shot by Nine-Tailed Fox.
									
									;player killed -> "target terminated"
									If prev => 0 And KillTimer < 0 Then
										DeathMSG="Субъект D-9341. Уничтожен Девятихвостой Лисой." ;Subject D-9341. Terminated by Nine-Tailed Fox.
		...
		If Rnd(1.0) =< hitProb Then
			...
			If WearingVest>0 Then
				If WearingVest = 1 Then
					Select Rand(8)
						Case 1,2,3,4,5
							BlurTimer = 500
							Stamina = 0
							ShotMessageUpdate = "Пуля проникла в жилет, заставив вас задохнуться." ;A bullet penetrated your vest, making you gasp.
							Injuries = Injuries + Rnd(0.1,0.5)
						Case 6
							BlurTimer = 500
							ShotMessageUpdate = "Пуля попала в вашу левую ногу." ;A bullet hit your left leg.
							Injuries = Injuries + Rnd(0.8,1.2)
						Case 7
							BlurTimer = 500
							ShotMessageUpdate = "Пуля попала в вашу левую ногу." ;A bullet hit your right leg.
							Injuries = Injuries + Rnd(0.8,1.2)
						Case 8
							BlurTimer = 500
							Stamina = 0
							ShotMessageUpdate = "Пуля попала в вашу шею, заставив вас задохнуться." ;A bullet struck your neck, making you gasp.
				...
				Else
					If Rand(10)=1 Then
						BlurTimer = 500
						Stamina = Stamina - 1
						ShotMessageUpdate = "Поля попала в вашу грудь. Жилет поглотил часть урона." ;A bullet hit your chest. The vest absorbed some of the damage.
			...
			Else
				Select Rand(6)
					...
					Case 4
						BlurTimer = 500
						ShotMessageUpdate = "Пуля попала в ваше правое плечо." ;A bullet hit your right shoulder.
						Injuries = Injuries + Rnd(0.8,1.2)	
					Case 5
						BlurTimer = 500
						ShotMessageUpdate = "Пуля попала в ваше левое плечо." ;A bullet hit your left shoulder.
						Injuries = Injuries + Rnd(0.8,1.2)	
					Case 6
						BlurTimer = 500
						ShotMessageUpdate = "Пуля попала в ваше правое плечо." ;A bullet hit your right shoulder.
	...
	For n = Each NPCs
		If n\ManipulateBone
			...
			If bonename$<>""
				...
				If bone% = 0 Then RuntimeError "ОШИБКА: NPC "+Chr(34)+bonename$+Chr(34)+" не существует." ;ERROR: NPC bone ;does not exist.
