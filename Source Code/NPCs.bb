									...
									Else
										...
										If n\Target=Null Then
											If (Not GodMode) Then 
												...
												CameraShake = 30
												BlurTimer = 2000
												DeathMSG = "В [ДАННЫЕ УДАЛЕНЫ] было найдено большое количество крови. ДНК принадлежит субъекту D-9341. Скорее всего, он был [ДАННЫЕ УДАЛЕНЫ] SCP-096." ;A large amount of blood found in [DATA REDACTED]. DNA indentified as Subject D-9341. Most likely [DATA REDACTED] by SCP-096.
												Kill()
												KillAnim = 1
												For i = 0 To 6
								...
								If PlayerSeeAble%=True Or n\State2>0 Then ;Player is visible for 049's sight - attacking
									...
									If dist < 0.5 Then
										...
										Else
											...
											If (Not GodMode) Then
												If PlayerRoom\RoomTemplate\Name$ = "room049"
													DeathMSG = "В туннеле рядом с камерой SCP-049 были найдены три (3) активных образца SCP-049-2. Уничтожены Девятихвостой Лисой." ;Three (3) active instances of SCP-049-2 discovered in the tunnel outside SCP-049's containment chamber. Terminated by Nine-Tailed Fox.
													For e.events = Each Events
														If e\EventName = "room049" Then e\EventState=-1 : Exit
													Next
												Else
													DeathMSG = "В [УДАЛЕНО] был найден активный образец SCP-049-2. Уничтожен Девятихвостой Лисой." ;An active instance of SCP-049-2 was discovered in [REDACTED]. Terminated by Nine-Tailed Fox.
													Kill()
												EndIf
									...
									If prevFrame < 23 And n\Frame=>23 Then
										If EntityDistance(n\Collider,Collider)<1.1
											If (Abs(DeltaYaw(n\Collider,Collider))<=60.0)
												...
												DeathMSG = "Субъект D-9341. Причина смерти: множественные раны и травмы, нанесённые SCP-049-2." ;Subject D-9341. Cause of death: multiple lacerations and severe blunt force trauma caused by an instance of SCP-049-2.
											EndIf
										EndIf
											...
											Else
												...
												If Injuries > 3.0 Then
													If PlayerRoom\RoomTemplate\Name = "room2offices" Then ;..!
														DeathMSG = Chr(34)+"Похоже, что одно большое и очень активное щупальце " ;One large and highly active tentacle-like appendage seems 
														DeathMSG = DeathMSG + "выросло из трупа учёного в офисе [УДАЛЕНО]. Его уровень агрессии " ;to have grown outside the dead body of a scientist within office area [REDACTED]. Its level of aggression is 
									...
									If ForestNPCData[2]=0
										...
										For i = 0 To MaxItemAmount-1
											If Inventory(i)<>Null
												...
												If docname = "Запись №1" Or docname = "Запись №2" Or docname = "Запись №3" ;Log #
													;860,850,830,800
							...
							If n\LastSeen > 0 And (Not NoTarget) Then
								...
								If (n\Frame=>18.0 And n\Frame<68.0) Then
									...
									If Injuries>4.0 Then 
										DeathMSG=Chr(34)+"Все четверо (4) сбежавших экземпляров SCP-939 были захвачены и успешно возвращены в должные условия содержания. " ;All four (4) escaped SCP-939 specimens have been captured and recontained successfully. 
					...
					If (WearingNightVision=0) Then
						...
						If dist<1 And n\Reload <= 0 And MsgTimer <= 0 Then
							Select Rand(6)
								...
								Case 2
									Msg=Chr(34)+"Такое ощущение, как будто рядом со мной что-то есть."+Chr(34) ;It feels like something's in this room with me.
								Case 3
									...
									Else
										...
										If Injuries > 10.0
											...
											If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
												...
												DeathMSG = DeathMSG + "вскоре был застрелен агентом [УДАЛЕНО]." ;died shortly after being shot by Agent [REDACTED].
											Else
												DeathMSG = "Неопознанный субъект мужского пола и мёртвый сотрудник класса D были обнаружены в [УДАЛЕНО] Девятихвостой Лисой. " ;An unidentified male and a deceased Class D subject were discovered in [REDACTED] by the Nine-Tailed Fox. 
									...
									Else
										...
										If Injuries > 10.0
											...
											If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
												DeathMSG = "Всему персоналу, располагавшемуся в эвакуационном убежище LC-2 во время катастрофы, были введены " ;All personnel situated within Evacuation Shelter LC-2 during the breach have been administered 
												DeathMSG = DeathMSG + "амнезиаки класса B в связи с Инцидентом 1499-E. Субъект класса D, участвовавший в этом происшествии, " ;Class-B amnestics due to Incident 1499-E. The Class D subject involved in the event 
												DeathMSG = DeathMSG + "был застрелен агентом [УДАЛЕНО]." ;died shortly after being shot by Agent [REDACTED].
											Else
												DeathMSG = "Неопознанный субъект мужского пола и мёртвый сотрудник класса D были обнаружены в [УДАЛЕНО] Девятихвостой Лисой. " ;An unidentified male and a deceased Class D subject were discovered in [REDACTED] by the Nine-Tailed Fox. 
												DeathMSG = DeathMSG + "Человек, как было отмечено, был весьма возбуждён и, похоже, говорил только по-русски. " ;The man was described as highly agitated and seemed to only speak Russian. 
												DeathMSG = DeathMSG + "Он был отправлен во временную зону сдерживания в [УДАЛЕНО] до прибытия переводчика." ;He's been taken into a temporary holding area at [REDACTED] while waiting for a translator to arrive.
...
Function Console_SpawnNPC(c_input$, c_state$ = "")
	...
	Select c_input$ 
		...
		Case "513-1", "5131", "scp513-1", "scp-513-1"
			...
			consoleMSG = "SCP-513-1 создан." ;SCP-513-1 spawned.
			
		Case "860-2", "8602", "scp860-2", "scp-860-2"
			CreateConsoleMsg("SCP-860-2 не может быть создан с помощью консоли. Простите!", 255, 0, 0) ;SCP-860-2 cannot be spawned with the console. Sorry!
			
		Case "939", "scp939", "scp-939"
			CreateConsoleMsg("Особи SCP-939 не могут быть созданы с помощью консоли. Простите!", 255, 0, 0) ;SCP-939 instances cannot be spawned with the console. Sorry!

		Case "966", "scp966", "scp-966"
			n.NPCs = CreateNPC(NPCtype966, EntityX(Collider), EntityY(Collider) + 0.2, EntityZ(Collider))
			consoleMSG = "Особи SCP-966 созданы." ;SCP-966 instance spawned.
		...
		Case "clerk"
			...
			consoleMSG = "Клерк создан." ;Clerk spawned.
			
		Default 
			CreateConsoleMsg("Тип NPC не найден.", 255, 0, 0) : Return ;NPC type not found.
	End Select
	
	If n <> Null
		If c_state <> "" Then n\State = Float(c_state) : consoleMSG = consoleMSG + " (Состояние = " + n\State + ")" ; (State = 
	EndIf
