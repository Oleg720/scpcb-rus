...
Font3% = AALoadFont("GFX\font\LCDNovaRus.ttf", Int(22 * (GraphicHeight / 1024.0)), 0,0,0)
...
ConsoleFont% = AALoadFont("Arial Cyr", Int(20 * (GraphicHeight / 1024.0)), 0,0,0,1) ;Blitz
		...
		If KeyHit(28) And ConsoleInput <> "" Then
			...
			Select Lower2(StrTemp)
				Case "help"
					...
					Select Lower2(StrTemp)
						...
						Case "revive","undead","resurrect"
							...
							CreateConsoleMsg("или инфекцию 008.") ;or 008 infection values.
							CreateConsoleMsg("******************************")
						Case "teleport"
						...
						Default
							CreateConsoleMsg("Для этой команды справка недоступна.",255,150,0) ;There is no help available for that command.
					End Select
				...
				Case "scp-420-j","420","weed"
					...
					For i = 1 To 20
						If Rand(2)=1 Then
							it.Items = CreateItem("Немного SCP-420-J","420", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5)) ;Some SCP-420-J
						Else
							it.Items = CreateItem("Самокрутка","420s", EntityX(Collider,True)+Cos((360.0/20.0)*i)*Rnd(0.3,0.5), EntityY(Camera,True), EntityZ(Collider,True)+Sin((360.0/20.0)*i)*Rnd(0.3,0.5)) ;Joint
						EndIf
						EntityType (it\collider, HIT_ITEM)
					Next
					...
					Select StrTemp
						Case "a"
							For e.Events = Each Events
								...
								EndIf
							Next
							CreateConsoleMsg("Ворота A разблокированы.") ;Gate A is now unlocked.
						Case "b"
							For e.Events = Each Events
								...
								EndIf
							Next	
							CreateConsoleMsg("Ворота B разблокированы.")	;Gate B is now unlocked.
						Default
							For e.Events = Each Events
								If e\EventName = "gateaentrance" Then
									e\EventState3 = 1
								...
								EndIf
							Next
							CreateConsoleMsg("Ворота A и B разблокированы.") ;Gate A and B are now unlocked.
					End Select
				...
				Case "kill","suicide"
					...
					Select Rand(4)
						...
						Case 2
							...
							DeathMSG = DeathMSG + "Похоже, что субъект не получил физического повреждение, и нет никаких видимых признаков того, что именно его убило. " ;The subject appears to have attained no physical damage, and there is no visible indication as to what killed him. 
							DeathMSG = DeathMSG + "Тело отправлено на вскрытие." ;Body was sent for autopsy.
						Case 3
							DeathMSG = "EXCP_ACCESS_VIOLATION"
						Case 4
							DeathMSG = "Суъект D-9341 найден мёртвым в секторе [УДАЛЕНО]. " ;Subject D-9341 found dead in Sector [REDACTED].
							DeathMSG = DeathMSG + "Похрже, что субъект написал буквы "+Chr(34)+"kys"+Chr(34)+" своей собственной кровью радом с собой. " ;The subject appears to have scribbled the letters "+Chr(34)+"kys"+Chr(34)+" in his own blood beside him.
							DeathMSG = DeathMSG + "Никаких других признаких физического повреждения или борьбы не надлюбается Тело отпралено на вскрытие." ;No other signs of physical trauma or struggle can be observed. Body was sent for autopsy
					End Select
					;[End Block]
				Case "playmusic"
					;[Block]
				...
				Case "spawnradio"
					;[Block]
					it.Items = CreateItem("Радио", "fineradio", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider)) ;Radio Transceiver
					EntityType(it\collider, HIT_ITEM)
					it\state = 101
					;[End Block]
				Case "spawnnvg"
					;[Block]
					it.Items = CreateItem("Очки ночного виденья", "nvgoggles", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider)) ;Night Vision Goggles
					EntityType(it\collider, HIT_ITEM)
					it\state = 1000
					;[End Block]
				...
				Case "spawnnav"
					;[Block]
					it.Items = CreateItem("Навигатор S-NAV Ultimate", "nav", EntityX(Collider), EntityY(Camera,True), EntityZ(Collider)) ;S-NAV Navigator Ultimate
					EntityType(it\collider, HIT_ITEM)
					it\state = 101
					;[End Block]
				Case "teleport173"
				...
				Case "spawnparticles"
					...
					If Int(StrTemp) > -1 And Int(StrTemp) <= 1 ;<--- This is the maximum ID of particles by Devil Particle system, will be increased after time - ENDSHN
						...
						CreateConsoleMsg("Создан эмитатор частиц с ID "+Int(StrTemp)+" на месте игрока.") ;Spawned particle emitter with ;at player's position.
					Else
						CreateConsoleMsg("Эмитатор частиц с ID "+Int(StrTemp)+" не найден.",255,150,0) ;Particle emitter with ;not found.
					EndIf
					;[End Block]
				Case "giveachievement"
					...
					If Int(StrTemp)>=0 And Int(StrTemp)<MAXACHIEVEMENTS
						...
						CreateConsoleMsg("Разблокировано достижение: "+AchievementStrings(Int(StrTemp))) ;Achievemt ;unlocked.
					Else
			...
			ElseIf temp >= d\KeyCard 
				...
				If showmsg = True Then
					...
					Else
						...
						Msg = "Ключ-карта вставлена в слот."; The keycard was inserted into the slot.
						MsgTimer = 70 * 7	
					EndIf
				EndIf
			Else
	...
	ElseIf d\KeyCard < 0
		...
		Else
			If showmsg = True Then 
				...
				Msg = "Вы положили вашу ладонь на сканер. Сканер написал: "+Chr(34)+"ДНК не соответствуют образцу. В доступе отказано."+Chr(34) ;You placed your palm onto the scanner. The scanner reads: "+Chr(34)+"DNA does not match known sample. Access denied.
				MsgTimer = 70 * 10
			EndIf
			Return			
		EndIf
	...
	Else
		...
		If KeyHit(KEY_SAVE) Then
			...
			ElseIf SelectedDifficulty\saveType = SAVEONSCREENS
				If SelectedScreen=Null And SelectedMonitor=Null Then
					Msg = "Сохраняться можно только у мониторов, разбросанных по всему комплексу." ;Saving is only permitted on clickable monitors scattered throughout the facility.
					MsgTimer = 70 * 4
		...
		If MsgTimer > 0 Then
			...
			If (Not temp%)
				...
				If Left(Msg,14)="Ошибка Blitz3D!" Then ;Blitz3D Error!
					Color 255,0,0
				EndIf
		...
		Color 255, 255, 255
		If ShowFPS Then AASetFont BlitzFont : AAText 20, 20, "FPS: " + FPS : AASetFont Font1
...
Function InitCredits()
	...
	CreditsFont% = LoadFont_Strict("GFX\font\cour\Courier New Rus.ttf", Int(21 * (GraphicHeight / 1024.0)), 0,0,0)
	CreditsFont2% = LoadFont_Strict("GFX\font\courbd\Courier New Rus.ttf", Int(35 * (GraphicHeight / 1024.0)), 0,0,0)
...
Function MovePlayer()
	...
	If SuperMan Then
		...
		If SuperManTimer > 70 * 50 Then
			DeathMSG = "Комбинезон класса D был найден в [ДАННЫЕ УДАЛЕНЫ]. При дальнейшей экспертизе выяснилось, что комбинезон был наполнен 12.5 килограммами субстанции в виде синего пепла. " ;A Class D jumpsuit found in [DATA REDACTED]. Upon further examination, the jumpsuit was found to be filled with 12.5 kilograms of blue ash-like substance. 
			DeathMSG = DeathMSG + "Химический анализ субстанции остаётся неокончательным. Скорее всего, всё это связано с SCP-914." ;Chemical analysis of the substance remains non-conclusive. Most likely related to SCP-914.
			Kill()
			ShowEntity Fog
		Else
			BlurTimer = 500		
...
Function MouseLook()
	...
	If KillTimer >= 0 And FallTimer >=0 Then
		...
		;kГ¤Г¤nnetГ¤Г¤n kameraa sivulle jos pelaaja on vammautunut
			...
			Select i
				...
				Case 3 ;appendicitis
					...
					If SCP1025state[i]>20.0 Then
						If SCP1025state[i]-FPSfactor<=20.0 Then Msg="Боль в жилудке становится невыносимой." ;The pain in your stomach is becoming unbearable.
						Stamina = Stamina - FPSfactor * 0.3
					ElseIf SCP1025state[i]>10.0
						If SCP1025state[i]-FPSfactor<=10.0 Then Msg="Вы чувствуете боль в жилудке." ;Your stomach is aching.
					EndIf
				Case 4 ;asthma
					If Stamina < 35 Then
				...
				Case 5;cardiac arrest
					...
					If SCP1025state[i]>110 Then
						...
						If SCP1025state[i]>140 Then 
							...
							DeathMSG = DeathMSG + "аномальные свойства, о которых мы ещё не в курсе?"+Chr(34) ;anomalous properties we are not yet aware of?
							Kill()
						EndIf
					Else
...
Function DrawGUI()
	...
	If HUDenabled Then 
		...
		If DebugHUD Then ;Если и переводить, то в последнюю очередь.
			Color 255, 255, 255
			AASetFont ConsoleFont
			...
			For ev.Events = Each Events
				...
				EndIf
			Next
			AAText x - 50, 280, "Координаты комнаты: (" + Floor(EntityX(PlayerRoom\obj) / 8.0 + 0.5) + ", " + Floor(EntityZ(PlayerRoom\obj) / 8.0 + 0.5) + ", угол: "+PlayerRoom\angle + ")" ;Room coordinates: ;angle:
			AAText x - 50, 300, "Выносливость: " + f2s(Stamina, 3) ;Stamina:
			AAText x - 50, 320, "Таймер смерти: " + f2s(KillTimer, 3) ;Death timer:      
			AAText x - 50, 340, "Таймер моргания: " + f2s(BlinkTimer, 3) ;Blink timer:
			AAText x - 50, 360, "Травмы: " + Injuries ;Injuries:
			AAText x - 50, 380, "Кровопотеря: " + Bloodloss ;Bloodloss:
			If Curr173 <> Null
				AAText x - 50, 410, "Позиция SCP - 173 (коллайдер): (" + f2s(EntityX(Curr173\Collider), 3) + ", " + f2s(EntityY(Curr173\Collider), 3) + ", " + f2s(EntityZ(Curr173\Collider), 3) + ")" ;Position (collider)
				AAText x - 50, 430, "Позиция SCP - 173 (obj): (" + f2s(EntityX(Curr173\obj), 3) + ", " + f2s(EntityY(Curr173\obj), 3) + ", " + f2s(EntityZ(Curr173\obj), 3) + ")" ;Position
				;Text x - 50, 410, "SCP - 173 Idle: " + Curr173\Idle
				AAText x - 50, 450, "Состояние SCP - 173: " + Curr173\State ;SCP - 173 State:
			EndIf
			If Curr106 <> Null
				AAText x - 50, 470, "Позиция SCP - 106: (" + f2s(EntityX(Curr106\obj), 3) + ", " + f2s(EntityY(Curr106\obj), 3) + ", " + f2s(EntityZ(Curr106\obj), 3) + ")" ;Position:
				AAText x - 50, 490, "Ожидание SCP - 106:" + Curr106\Idle ;Idle:
				AAText x - 50, 510, "Состояние SCP - 106: " + Curr106\State ;State:
			EndIf
			offset% = 0
			For npc.NPCs = Each NPCs
				If npc\NPCtype = NPCtype096 Then
					AAText x - 50, 530, "Позиция SCP - 096: (" + f2s(EntityX(npc\obj), 3) + ", " + f2s(EntityY(npc\obj), 3) + ", " + f2s(EntityZ(npc\obj), 3) + ")" ;Position: 
					AAText x - 50, 550, "Ожидание SCP - 096: " + npc\Idle ;Idle:
					AAText x - 50, 570, "Состояние SCP - 096: " + npc\State ;State:
					AAText x - 50, 590, "Скорость SCP - 096: " + f2s(npc\currspeed, 5) ;Speed:
				EndIf
				If npc\NPCtype = NPCtypeMTF Then
					AAText x - 50, 620 + 60 * offset, "MTF (МОГ) " + offset + " Позиция: (" + f2s(EntityX(npc\obj), 3) + ", " + f2s(EntityY(npc\obj), 3) + ", " + f2s(EntityZ(npc\obj), 3) + ")" ;MTF " + offset + " Position:
					AAText x - 50, 640 + 60 * offset, "МTF (МОГ) " + offset + " Состояние: " + npc\State ;MTF " + offset + " State:
					AAText x - 50, 660 + 60 * offset, "MTF (МОГ) " + offset + " ПоследняяВстреча: " + npc\lastseen ;MTF " + offset + " LastSeen: 		
					offset = offset + 1
				EndIf
			Next
			If PlayerRoom\RoomTemplate\Name$ = "dimension1499"
			...
			If SelectedMonitor <> Null Then
				AAText x + 350, 310, "Текущий монитор: "+SelectedMonitor\ScrObj ;Current monitor:
			Else
			...
			Else
				...
				Else
					...
					ElseIf PrevOtherOpen\SecondInv[MouseSlot] <> SelectedItem
						Select SelectedItem\itemtemplate\tempname
							Default
								Msg = "Вы не можете объеденить два этих предмета." ;You cannot combine these two items.
								MsgTimer = 70 * 5
						End Select					
					EndIf
						...
						Select SelectedItem\itemtemplate\tempname
							Case "paper","key1","key2","key3","key4","key5","key6","misc","oldpaper","badge","ticket","50ct","coin","key","scp860","scp714"
								...
								If Inventory(MouseSlot)\itemtemplate\tempname = "clipboard" Then
									...
									If (b<>"misc" And b<>"50ct" And b<>"coin" And b<>"key" And b<>"scp860" And b<>"scp714") Or (b2="Игральная карта" Or b2="Mastercard") Then ;Playing Card
								...
								ElseIf Inventory(MouseSlot)\itemtemplate\tempname = "wallet" Then
									...
									If (b<>"misc" And b<>"paper" And b<>"oldpaper") Or (b2="Игральная карта" Or b2="Mastercard") Then ;Playing Card
										For c% = 0 To Inventory(MouseSlot)\invSlots-1
											If (Inventory(MouseSlot)\SecondInv[c] = Null)
												If SelectedItem <> Null Then
													Inventory(MouseSlot)\SecondInv[c] = SelectedItem
										...
										Next
										If SelectedItem <> Null Then
											Msg = "Кошелёк полный." ;The wallet is full.
										Else
							...
							Case "battery", "bat"
								...
								Select Inventory(MouseSlot)\itemtemplate\name
									...
									Case "Навигатор S-NAV Ultimate" ;S-NAV Navigator Ultimate
										Msg = "Кажется, в этом навигаторе нет отсека для батареек." ;There seems to be no place for batteries in this navigator.
										MsgTimer = 70 * 5
									Case "Рация" ;Radio Transceiver
										Select Inventory(MouseSlot)\itemtemplate\tempname 
											Case "fineradio", "veryfineradio"
												Msg = "Кажется, в этой рации нет отсека для батареек." ;There seems to be no place for batteries in this radio.
												MsgTimer = 70 * 5
											Case "18vradio"
												Msg = "Батарейка не помещается внутри этой рации." ;The battery does not fit inside this radio.
												MsgTimer = 70 * 5
											Case "radio"
												If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(PickSFX(SelectedItem\itemtemplate\sound))	
												RemoveItem (SelectedItem)
									...
									Case "Очки ночного виденья" ;Night Vision Goggles
										...
										If nvname$="nvgoggles" Or nvname$="supernv" Then
											...
											MsgTimer = 70 * 5
										Else
											Msg = "Кажется, в этих очках ночного видения нет отсека для батареек." ;There seems to be no place for batteries in these night vision goggles.
											MsgTimer = 70 * 5
										EndIf
									Default
										Msg = "Вы не можете объеденить два этих предмета.." ;You cannot combine these two items.
							...
							Case "18vbat"
								...
								Select Inventory(MouseSlot)\itemtemplate\name
									Case "Навигатор S-NAV", "Навигатор S-NAV 300", "Навигатор S-NAV 310" ;S-NAV Navigator ;S-NAV 300 Navigator ;S-NAV 310 Navigator
										Msg = "Батарейка не помещается внутри этого навигатора." ;The battery does not fit inside this navigator.
										MsgTimer = 70 * 5
									Case "Навигатор S-NAV Ultimate" ;S-NAV Navigator Ultimate
										Msg = "Кажется, в этом навигаторе нет отсека для батареек." ;The battery does not fit inside this navigator.
										MsgTimer = 70 * 5
									Case "Рация" ;Radio Transceiver
										Select Inventory(MouseSlot)\itemtemplate\tempname 
											Case "fineradio", "veryfineradio"
			...
			Select SelectedItem\itemtemplate\tempname
							...
							Case 5
								...
								Else
									For r.Rooms = Each Rooms
										...
										EndIf
									Next
									Msg = "По необъясникой причине, вы оказались внутри карманного измерения." ;For some inexplicable reason. You find yourself inside the pocket dimension.
									MsgTimer = 70*8
								EndIf
						End Select
							...
							If SelectedItem\state = 100 Then
								...
								Else
									...
									If Injuries => 2.5 Then
										Msg = "Раны были слишком большими, чтобы остановить кроотечение." ;The wounds were way too severe to staunch the bleeding completely.
									...
									Else
										If Injuries > 0.5 Then
											Injuries = 0.5
											Msg = "Вы приняли обезболивающее, слегка облегчив боль." ;You took a painkiller, easing the pain slightly."
										Else
											Injuries = 0.5
				...
				Case "paper", "ticket"
					...
					If SelectedItem\itemtemplate\img = 0 Then
						Select SelectedItem\itemtemplate\name
							Case "Разъеденный документ" ;Burnt Note
								...
								Color 255,255,255
								SetBuffer BackBuffer()
							Case "Документ об SCP-372" ;Document SCP-372
								...
								Color 255,255,255
								SetBuffer BackBuffer()
							Case "Билет в кино" ;Movie Ticket
				...
				Case "cup" ;..?
					...
					If CanUseItem(False,False,True)
						...
						If strtemp <> "" Then
							...
							MsgTimer = 70*6		
						Else
							it.Items = CreateItem("Пустой стакан", "emptycup", 0,0,0) ;Empty Cup
							it\Picked = True
							For i = 0 To MaxItemAmount-1
				...
				Case "syringe"
					...
					If CanUseItem(False,True,True)
						...
						Msg = "Вы вкололи себе шприц, и почувствовали лёгкий прилив адреналина." ;You injected yourself with the syringe and feel a slight adrenaline rush.
						MsgTimer = 70 * 8
					...
					If CanUseItem(False,True,True)
						Select Rand(3)
							Case 1
								...
								Msg = "Вы вкололи себе шприц, и почувствовали сильный прилив адреналина." ;You injected yourself with the syringe and feel a huge adrenaline rush.
							Case 2
								SuperMan = True
								Msg = "Вы вкололи себе шприц, и почувствовали чрезмерный прилив адреналина." ;You injected yourself with the syringe and feel a humongous adrenaline rush.
							Case 3
								VomitTimer = 30
								Msg = "Вы вкололи себе шприц, и почувствовали боль в животе." ;You injected yourself with the syringe and feel a pain in your stomach.
						End Select
							...
							Else
								For i = 2 To 6
									If KeyHit(i) Then
										...
										;jos nykyistГ¤ kanavaa ollaan soitettu, laitetaan jatketaan toistoa samasta kohdasta
				...
				Case "cigarette"
					...
					If CanUseItem(False,False,True)
						If SelectedItem\state = 0 Then
							Select Rand(6)
								Case 1
									Msg = Chr(34)+"Мне нечем поджечь её. Хмм, как насчёт... Неважно."+Chr(34) ;I don't have anything to light it with. Umm, what about that... Nevermind.
								Case 2
									Msg = "Вам нечем зажечь это." ;You are unable to get lit.
								Case 3
									Msg = Chr(34)+"Я бросил давным-давно."+Chr(34) ;I quit that a long time ago.
									RemoveItem(SelectedItem)
								Case 4
									Msg = Chr(34)+"Даже если бы я захотел. Мне всё равно нечем это зажечь."+Chr(34) ;Even if I wanted one, I have nothing to light it with.
								Case 5
									Msg = Chr(34)+"Одна не помешала бы... Жаль, что у меня нет зажигалки."+Chr(34) ;Could really go for one now... Wish I had a lighter.
								Case 6
									Msg = Chr(34)+"Не планирую начинать, даже в такое время."+Chr(34) ;Don't plan on starting, even at a time like this.
									RemoveItem(SelectedItem)
							End Select
							SelectedItem\state = 1 
						Else
							Msg = "Вам нечем зажечь это."
						EndIf
				...
				Case "420s"
					...
					If CanUseItem(False,False,True)
						If Wearing714=1 Then
							Msg = Chr(34) + "ЧУВАК, ЧЗХ, ЭТА ХРЕНЬ НЕ РАБОТАЕТ" + Chr(34) ;DUDE WTF THIS SHIT DOESN'T EVEN WORK
						Else
							DeathMSG = "Субъект D-9341 найден в коматозном состоянии в [ДАННЫЕ УДАЛЕНЫ]. Субъект держал предмет, похожий на сигарету, и широко улыбался. " ;Subject D-9341 found in a comatose state in [DATA REDACTED]. The subject was holding what appears to be a cigarette while smiling widely. 
							DeathMSG = DeathMSG+"Химический анализ сигареты был безрезультатным, но похоже, что она содержит высокую концентрацию неизвестных химикатов, " ;Chemical analysis of the cigarette has been inconclusive, although it seems to contain a high concentration of an unidentified chemical 
							...
							Msg = Chr(34) + "Э-Э, ГДЕ... ЧТО ЭТО Я СДЕЛАЛ... ЧУВАК, МНЕ НУЖНО ПОСПАТЬ..." + Chr(34) ;UH WHERE... WHAT WAS I DOING AGAIN... MAN I NEED TO TAKE A NAP...
							KillTimer = -1						
						EndIf
						MsgTimer = 70 * 6
						RemoveItem(SelectedItem)
					...
					If SelectedItem\state=100 Then
						...
						Else
							If SelectedItem\itemtemplate\tempname="vest" Then
								Msg = "Вы надели бронижилет, и почувствовали себя слегка обременённым." ;You put on the vest and feel slightly encumbered.
								WearingVest = 1
							Else
								Msg = "Вы надели бронижилет, и почувствовали себя очень обременённым." ;You put on the vest and feel heavily encumbered.
								WearingVest = 2
							EndIf
							If SelectedItem\itemtemplate\sound <> 66 Then PlaySound_Strict(PickSFX(SelectedItem\itemtemplate\sound))
							TakeOffStuff(2)
				...
				Case "navigator", "nav"
					...
					Else
						...
						If SelectedItem\state > 0 And (Rnd(CoffinDistance + 15.0) > 1.0 Or PlayerRoom\RoomTemplate\Name <> "coffin") Then
							...
							If SelectedItem\itemtemplate\name = "Навигатор S-NAV" Then  ;S-NAV Navigator
								Color(100, 0, 0)
							Else
								Color (30,30,30)
							EndIf
							...
							Local SCPs_found% = 0
							If SelectedItem\itemtemplate\name = "Навигатор S-NAV Ultimate" And (MilliSecs2() Mod 600) < 400 Then ;S-NAV Navigator Ultimate
				...
				Case "scp1499","super1499"
					;[Block]
					If WearingHazmat>0
						Msg = "Вы не можете носить SCP-1499 и Защитный костюм одноременно." ;You are not able to wear SCP-1499 and a hazmat suit at the same time.
						MsgTimer = 70 * 5
						SelectedItem=Null
						Return
					EndIf
				...
				Case "oldpaper"
					...
					If SelectedItem\state = 0
						Select SelectedItem\itemtemplate\name
							Case "Дисциплинарное слушание DH-S-4137-17092" ;Disciplinary Hearing DH-S-4137-17092
								BlurTimer = 1000
...
Function DrawMenu()
	...
	If MenuOpen Then
			...
			ElseIf StopHidingTimer < 40
				If KillTimer >= 0 Then 
					...
					If StopHidingTimer => 40 Then
						...
						Msg = "ХВАТИТ ПРЯТАТЬСЯ" ;STOP HIDING
						MsgTimer = 6*70
						MenuOpen = False
						Return
					EndIf
		...
		If AchievementsMenu > 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "ДОСТИЖЕНИЯ",False,True) ;ACHIEVEMENTS
			AASetFont Font1
		...
		ElseIf QuitMSG > 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "ВЫЙТИ?",False,True) ;QUIT?
			AASetFont Font1
		ElseIf KillTimer >= 0 Then
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "ПАУЗА",False,True) ;PAUSED
			AASetFont Font1
		Else
			AASetFont Font2
			AAText(x, y-(122-45)*MenuScale, "ВЫ ПОГИБЛИ",False,True) ;YOU DIED
			AASetFont Font1
		End If		
		...
		If AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG <= 0
			AASetFont Font1
			AAText x, y, "Сложность: "+SelectedDifficulty\name ;Difficulty:
			AAText x, y+20*MenuScale, "Сохранение: "+CurrSave ;Save
			AAText x, y+40*MenuScale, "Сид карты: "+RandomSeed ;Map seed
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			If DrawButton(x + 101 * MenuScale, y + 390 * MenuScale, 230 * MenuScale, 60 * MenuScale, "Назад") Then ;Back
				AchievementsMenu = 0
				OptionsMenu = 0
				QuitMSG = 0
				MouseHit1 = False
			...
			Select OptionsMenu
				...
				Case 2 ;Audio
					...
					Color 255,255,255
					AAText(x, y, "Громкость музыки:") ;Music volume:
					...
					Color 255,255,255
					AAText(x, y, "Громкость звуков:") ;Sound volume:
					If MouseOn(x+250*MenuScale,y-4*MenuScale,100*MenuScale+14,20)
					...
					Color 100,100,100
					AAText x, y, "Авто-высвобождение звука:" ;Sound auto-release:
					...
					Color 100,100,100
					AAText x, y, "Включить польз. треки:" ;Enable user tracks:
					...
					If EnableUserTracks
						y = y + 30 * MenuScale
						Color 255,255,255
						AAText x, y, "Режим польз. треков:" ;User track mode:
						UserTrackMode = DrawTick(x + 270 * MenuScale, y + MenuScale, UserTrackMode)
						If UserTrackMode
							AAText x, y + 20 * MenuScale, "Повтор" ;Repeat
						Else
							AAText x, y + 20 * MenuScale, "Случайно" ;Random
						EndIf
						If MouseOn(x+270*MenuScale,y+MenuScale,20*MenuScale,20*MenuScale)
				...
				Case 4 ;Advanced
					...
					If AATextEnable_Prev% <> AATextEnable
						...
						EndIf
						InitAAFont()
						Font1% = AALoadFont("GFX\font\cour\Courier New Rus.ttf", Int(18 * (GraphicHeight / 1024.0)), 0,0,0)
						Font2% = AALoadFont("GFX\font\courbd\Courier New Rus.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
						Font3% = AALoadFont("GFX\font\LCDNovaRus.ttf.ttf", Int(22 * (GraphicHeight / 1024.0)), 0,0,0)
						Font4% = AALoadFont("GFX\font\LCDNovaRus.ttf", Int(60 * (GraphicHeight / 1024.0)), 0,0,0)
						Font5% = AALoadFont("GFX\font\Journal\Journal.ttf", Int(58 * (GraphicHeight / 1024.0)), 0,0,0)
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu <= 0 And QuitMSG > 0 And KillTimer >= 0
			...
			If DrawButton(x+101*MenuScale, y + 344*MenuScale, 230*MenuScale, 60*MenuScale, "Назад") Then ;Back
				AchievementsMenu = 0
				OptionsMenu = 0
				QuitMSG = 0
				MouseHit1 = False
...
Function InitNewGame()
	...
	For r.Rooms = Each Rooms
		...
		If (r\RoomTemplate\Name = "start" And IntroEnabled = False) Then 
			...
			PlayerRoom = r
			it = CreateItem("Ориентационная листовка класса D", "paper", 1, 1, 1) ;Class D Orientation Leaflet
			it\Picked = True
			it\Dropped = -1
			...
			EntityParent(it\collider, 0)
			ItemAmount = ItemAmount + 1
			it = CreateItem("Документ об SCP-173", "paper", 1, 1, 1) ;Document SCP-173
			it\Picked = True
			it\Dropped = -1
			it\itemtemplate\found=True
			Inventory(1) = it
...
Function Use914(item.Items, setting$, x#, y#, z#)
	...
	Select item\itemtemplate\name
		...
		Case "Планшет" ;Clipboard
			Select setting
				...
				Case "very fine"
					...
					ResetEntity(item\collider)
			End Select
		Case "Коровий колокольчик" ;Cowbell
			Select setting
				Case "rough","coarse"
		...
		Case "Аптечка первой помощи", "Синяя аптечка первой помощи" ;First Aid Kit ;Blue First Aid Kit
			...
			End Select
			RemoveItem(item)
		Case "Ключ-карта 1-го уровня", "Ключ-карта 2-го уровня", "Ключ-карта 3-го уровня", "Ключ-карта 4-го уровня", "Ключ-карта 5-го уровня", "Ключ-карта" ;Level 1 Key Card ;Level 2 Key Card ;Level 3 Key Card ;Level 4 Key Card ;Level 5 Key Card Key Card
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
								...
								Select SelectedDifficulty\otherFactors
									Case EASY
										If Rand(0,((MAXACHIEVEMENTS-1)*3)-((CurrAchvAmount-1)*3))=0
											it2 = CreateItem("Ключ-карта Омни", "key6", x, y, z) ;Key Card Omni
										Else
											it2 = CreateItem("Mastercard", "misc", x, y, z)
										EndIf
									Case NORMAL
		...
		Case "Ключ-карта Омни" ;Key Card Omni
			...
			RemoveItem(item)
		Case "Игральная карта", "Монета", "50 центная монета" ;Playing Card ;Coin ;50 Cent Coin
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.005, z, 90, Rand(360), 0)
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
			    ...
			    Case "fine", "very fine"
					it2 = CreateItem("Ключ-карта 2-го уровня", "key2", x, y, z) ;Level 1 Key Card
			End Select
			RemoveItem(item)
		Case "Mastercard"
			Select setting
				Case "rough"
					...
					d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
				Case "coarse"
					it2 = CreateItem("50 центная монета", "50ct", x, y, z) ;50 Cent Coin
					Local it3.Items,it4.Items,it5.Items
					it3 = CreateItem("50 центная монета", "50ct", x, y, z) ;50 Cent Coin
					it4 = CreateItem("50 центная монета", "50ct", x, y, z) ;50 Cent Coin
					it5 = CreateItem("50 центная монета", "50ct", x, y, z) ;50 Cent Coin
		...
		Case "Немного SCP-420-J", "Сигарета" ;Some SCP-420-J ;Cigarette
			Select setting
				...
				Case "fine"
					it2 = CreateItem("Самокрутка", "420s", x + 1.5, y + 0.5, z + 1.0) ;Joint
				Case "very fine"
					it2 = CreateItem("Пахучая самокрутка", "420s", x + 1.5, y + 0.5, z + 1.0) ;Smelly Joint
			End Select
		...
		Case "9V батарейка", "18V батарейка", "Странная батарейка" ;9V Battery ;18V Battery ;Strange Battery
			...
			RemoveItem(item)
		Case "Глазные капли РеЗрение", "Красные глазные капли РеЗрение", "Глазные капли" ;ReVision Eyedrops ;RedVision Eyedrops ;Eyedrops
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("Красные глазные капли РеЗрение", "eyedrops", x,y,z) ;RedVision Eyedrops
				Case "fine"
				...
				Case "finesyringe"
					Select setting
						Case "rough"
							...
							d\Size = 0.07 : ScaleSprite(d\obj, d\Size, d\Size)
						Case "coarse"
							it2 = CreateItem("Маленькая аптечка первой помощи", "firstaid", x, y, z) ;First Aid Kit
						Case "1:1"
							it2 = CreateItem("Синяя аптечка первой помощи", "firstaid2", x, y, z) ;Blue First Aid Kit
						Case "fine", "very fine"
							it2 = CreateItem("Шприц", "veryfinesyringe", x, y, z) ;Syringe
		...
		Case "SCP-500-01", "Улучшенная пилюля", "Пилюля" ;Upgraded pill ;Pill
			Select setting
				...
				Case "fine"
					...
					Else
						it2 = CreateItem("Улучшенная пилюля", "scp500death", x, y, z) ;Upgraded pill
					EndIf
					RemoveItem(item)
				Case "very fine"
					it2 = CreateItem("Улучшенная пилюля", "scp500death", x, y, z) ;Upgraded pill
...
Function Use294()
		...
		If MouseHit1 Then
			...
			If temp And Input294<>"" Then ;dispense
				...
				If loc > 0 Then
					...
					it\name = "Стакан, в котором "+Input294 ;Cup of
					EntityType (it\collider, HIT_ITEM)
					
				Else
					;out of range
					Input294 = "ВНЕ ДИАПАЗОНА" ;OUT OF RANGE
	...
	Else ;playing a dispensing sound
		If Input294 <> "ВНЕ ДИАПАЗОНА" Then Input294 = "РАЗЛИВ..." ;OUT OF RANGE ;DISPENSING...
		
		If Not ChannelPlaying(PlayerRoom\SoundCHN) Then
			If Input294 <> "ВНЕ ДИАПАЗОНА" Then ;OUT OF RANGE
				HidePointer()
				Using294 = False
				MouseXSpeed() : MouseYSpeed() : MouseZSpeed() : mouse_x_speed_1#=0.0 : mouse_y_speed_1#=0.0
				Local e.Events
...
Function Use427()
	...
	If I_427\Timer < 70*360
		If I_427\Using=True Then
			...
			If prevI427Timer < 70*60 And I_427\Timer => 70*60 Then
				Msg = "Вы чувствуете себя отдохнувшим, и полны энергии." ;You feel refreshed and energetic.
				MsgTimer = 70*5
			ElseIf prevI427Timer < 70*180 And I_427\Timer => 70*180 Then
				Msg = "Вы чувствуете нежные мышечные спазмы по всему телу." ;You feel gentle muscle spasms all over your body.
				MsgTimer = 70*5
			EndIf
		Else
			For i = 0 To 1
	...
	Else
		...
		If I_427\Timer >= 70*420 Then
			Kill()
			DeathMSG = Chr(34)+"Запрашиваю поддержку от MTF Nu-7. Нам нужно больше огневой мощи, что уничтожить это."+Chr(34) ;Requesting support from MTF Nu-7. We need more firepower to take this thing down.
...
Function UpdateMTF%()
	...
	;mtf ei vielГ¤ spawnannut, spawnataan jos pelaaja menee tarpeeksi lГ¤helle gate b:tГ¤
	If MTFtimer = 0 Then
	...
	If Infect>0 Then
		...
		If Infect < 93.0 Then
			...
			ElseIf Infect > 80 And temp =< 80.0
				Msg = "Вы чувствуете слабость." ;You feel very faint.
				MsgTimer = 70*6
			ElseIf Infect =>91.5
		...
		Else
			...
			If teleportForInfect
				...
				ElseIf Infect < 98.5
					...
					If temp < 94.7 Then 
						...
						DeathMSG = "Субъект D-9341 поедал доктора [УДАЛЕНО] в секторе [УДАЛЕНО]. Субъект был немедленно уничтожен Девятихвостой Лисой и был направлен на вскрытие. " ;Subject D-9341 found ingesting Dr. [REDACTED] at Sector [REDACTED]. Subject was immediately terminated by Nine-Tailed Fox and sent for autopsy. 
			...
			Else
				...
				If PlayerRoom\RoomTemplate\Name = "dimension1499" Then
					DeathMSG = "Местонахождение SCP-1499 до сих пор неизвестно, однако разведгруппа отправлена на расследование сообщений о нападении с применением насилия на церковь в российском городе [УДАЛЕНО]." ;The whereabouts of SCP-1499 are still unknown, but a recon team has been dispatched to investigate reports of a violent attack to a church in the Russian town of [REDACTED].
				ElseIf PlayerRoom\RoomTemplate\Name = "gatea" Or PlayerRoom\RoomTemplate\Name = "exit1" Then
					DeathMSG = "Субъект D-9341 обнаружен блуждающим у Ворот " ;Subject D-9341 found wandering around Gate
					If PlayerRoom\RoomTemplate\Name = "gatea" Then
						DeathMSG = DeathMSG + "A"
					Else
						DeathMSG = DeathMSG + "B"
					EndIf
					DeathMSG = DeathMSG + ". Субъект был немедленно уничтожен Девятихвостой Лисой и был направлен на вскрытие. " ;Subject was immediately terminated by Nine-Tailed Fox and sent for autopsy. 
					DeathMSG = DeathMSG + "Инфекция SCP-008 была подтверждена, после чего тело было сожжено." ;SCP-008 infection was confirmed, after which the body was incinerated.
				Else
					DeathMSG = ""
				EndIf
...
Function CircleToLineSegIsect% (cx#, cy#, r#, l1x#, l1y#, l2x#, l2y#)
	...
	;Palauttaa:
	;  True (1) kun:
	;      YmpyrГ¤ [keskipiste = (cx, cy): sГ¤de = r]
	;      leikkaa janan, joka kulkee pisteiden (l1x, l1y) & (l2x, l2y) kaitta
	;  False (0) muulloin
	
	;YmpyrГ¤n keskipisteen ja (ainakin toisen) janan pГ¤Г¤tepisteen etГ¤isyys < r
	;-> leikkaus
	...
	;Vektorit (janan vektori ja vektorit janan pГ¤Г¤tepisteistГ¤ ympyrГ¤n keskipisteeseen)
	...
	;Janan pГ¤Г¤tepisteiden kautta kulkevan suoran ;yhtГ¤lГ¶; (ax + by + c = 0)
	Local a# = (l2y - l1y) / (l2x - l1x)
	Local b# = -1
	Local c# = -(l2y - l1y) / (l2x - l1x) * l1x + l1y
	
	;YmpyrГ¤n keskipisteen etГ¤isyys suorasta
	Local d# = Abs(a * cx + b * cy + c) / Sqr(a * a + b * b)
	
	;YmpyrГ¤ on liian kaukana
	;-> ei leikkausta
	If d > r Then Return False
	
	;Local kateetin_pituus# = Cos(angle) * hyp
	
	;Jos pГ¤Г¤stГ¤Г¤n tГ¤nne saakka, ympyrГ¤ ja jana leikkaavat (tai ovat sisГ¤kkГ¤in)
	Return True
End Function
...
Function TakeOffStuff(flag%=0)
	...
	If Len(numb_flag%)>1
		If Mid(numb_flag%,Len(numb_flag%)-1,1) = 1
			...
			For i = 0 To MaxItemAmount-1
				If Inventory(i) <> Null Then
					If Inventory(i)\itemtemplate\name = "Защитный костюм" Or Inventory(i)\itemtemplate\tempname = "hazmatsuit3" ;Hazmat Suit
...
Function PutINIValue%(file$, INI_sSection$, INI_sKey$, INI_sValue$)
	...
	INI_sSection = "[" + Trim2$(INI_sSection) + "]"
	Local INI_sUpperSection$ = Upper$(INI_sSection)
	INI_sKey = Trim2$(INI_sKey)
	INI_sValue = Trim2$(INI_sValue)
			...
			Else
				...
				Else
					...
					If (lEqualsPos <> 0) Then
						If (INI_sCurrentSection = INI_sUpperSection) And (Upper$(Trim2$(Left$(INI_sTemp, (lEqualsPos - 1)))) = Upper$(INI_sKey)) Then
...
Function RenderWorld2()
			...
			If (Inventory(i)<>Null) Then
				If (WearingNightVision = 1 And Inventory(i)\itemtemplate\tempname = "nvgoggles") Or (WearingNightVision = 2 And Inventory(i)\itemtemplate\tempname = "supernv") Then
					...
					If Inventory(i)\state<=0.0 Then ;this nvg can't be used
						hasBattery = 0
						Msg = "Батарейки в очках ночного видения разряжены." ;The batteries in these night vision goggles died.
						BlinkTimer = -1.0
						MsgTimer = 350
						Exit
	...
	If BlinkTimer < - 16 Or BlinkTimer > - 6
		If WearingNightVision=2 And hasBattery<>0 Then ;show a HUD
			...
			AAText GraphicWidth/2,(20+plusY)*MenuScale,"ОБНОВЛЕНИЕ ДАННЫХ ЧЕРЕЗ",True,False ;REFRESHING DATA IN
			
			AAText GraphicWidth/2,(60+plusY)*MenuScale,Max(f2s(NVTimer/60.0,1),0.0),True,False
			AAText GraphicWidth/2,(100+plusY)*MenuScale,"СЕКУНД",True,False ;SECONDS
	...
	If BlinkTimer < - 16 Or BlinkTimer > - 6
		If (WearingNightVision=1 Or WearingNightVision=2) And (hasBattery=1) And ((MilliSecs2() Mod 800) < 400) Then
			...
			AAText GraphicWidth/2,20*MenuScale,"ВНИМАНИЕ: НИЗКИЙ УРОВЕНЬ ЗАРЯДА",True,False ;WARNING: LOW BATTERY
			Color 255,255,255
		EndIf
	EndIf
End Function
...
Function CatchErrors(location$)
	...
	If Len(errStr)>0 Then
		If FileType(ErrorFile)=0 Then
			...
			WriteLine errF,"В SCP - Containment Breach произошла ошибка!" ;An error occured in SCP - Containment Breach!
			WriteLine errF,"Версия: "+VersionNumber ;Version:
			WriteLine errF,"Версия (Совместимость сохранений): "+CompatibleNumber ;Save compatible version:
			...
			WriteLine errF,"Треугольников отображено: "+CurrTrisAmount ;Triangles rendered
		...
		If canwriteError
			...
			Wend
		EndIf
		Msg = "Ошибка Blitz3D! Подробности в "+Chr(34)+ErrorFile+Chr(34)
		MsgTimer = 20*70
		CloseFile errF
	EndIf
End Function
...
Function CanUseItem(canUseWithHazmat%, canUseWithGasMask%, canUseWithEyewear%)
	...
	Else If (canUseWithEyewear = False And (WearingNightVision))
		Msg = "Вы не можете использовать этот предмет во время ношения головного убора." ;You can't use that item while wearing headgear.
	EndIf
