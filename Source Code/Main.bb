...
Global VersionNumber$ = "1.3.11 [Rus v1.311-TEST]"
	...
	ElseIf d\KeyCard < 0
		...
		Else
			If showmsg = True Then 
				...
				Msg = "Вы приложили свою ладонь к сканеру. Сообщение: "+Chr(34)+"ДНК не соответствуют образцу. В доступе отказано."+Chr(34) ;You placed your palm onto the scanner. The scanner reads: "+Chr(34)+"DNA does not match known sample. Access denied.
				MsgTimer = 70 * 10
			EndIf
			Return			
		EndIf
...
Repeat
	...
	Else
		...
		Color 255, 255, 255
		If ShowFPS Then AASetFont BlitzFont : AAText 20, 20, "FPS: " + FPS : AASetFont Font1 ;ConsoleFont
			...
			Select i
				...
				Case 3 ;appendicitis
					...
					If SCP1025state[i]>20.0 Then
						If SCP1025state[i]-FPSfactor<=20.0 Then Msg="Боль в желудке становится невыносимой." ;The pain in your stomach is becoming unbearable.
						Stamina = Stamina - FPSfactor * 0.3
					ElseIf SCP1025state[i]>10.0
						If SCP1025state[i]-FPSfactor<=10.0 Then Msg="Вы чувствуете боль в желудке." ;Your stomach is aching.
					EndIf
				Case 4 ;asthma
					If Stamina < 35 Then
						...
						Select SelectedItem\itemtemplate\tempname
							Case "paper","key1","key2","key3","key4","key5","key6","misc","oldpaper","badge","ticket","25ct","coin","key","scp860"
								...
								If Inventory(MouseSlot)\itemtemplate\tempname = "clipboard" Then
									...
									If (b<>"misc" And b<>"25ct" And b<>"coin" And b<>"key" And b<>"scp860" And b<>"scp714") Or (b2="Игральная карта" Or b2="Mastercard") Then ;Playing Card
								...
								ElseIf Inventory(MouseSlot)\itemtemplate\tempname = "wallet" Then
									...
									If (b<>"misc" And b<>"paper" And b<>"oldpaper") Or (b2="Игральная карта" Or b2="Mastercard") Then ;Playing Card
										...
										Next
										If SelectedItem <> Null Then
											Msg = "Кошелёк полон." ;The wallet is full.
										Else
			...
			Select SelectedItem\itemtemplate\tempname
				...
				Case "gasmask", "supergasmask", "gasmask3"
					...
					If Wearing1499 = 0 And WearingHazmat = 0 Then
						...
						EndIf
					ElseIf Wearing1499 > 0 Then
						Msg = "Нужно снять SCP-1499, чтобы надеть противогаз." ;You need to take off SCP-1499 in order to put on the gas mask.
					Else
						Msg = "Нужно снять защитный костюм, чтобы надеть противогаз." ;You need to take off the hazmat suit in order to put on the gas mask.
					EndIf
					SelectedItem = Null
					MsgTimer = 70 * 5
					;[End Block]
		...
		ElseIf AchievementsMenu <= 0 And OptionsMenu > 0 And QuitMSG <= 0 And KillTimer >= 0
			...
			Select OptionsMenu
				...
				Case 2 ;Audio
					...
					Color 100,100,100
					AAText x, y, "Автовысвобождение звука:" ;Sound auto-release:
...
Function Use914(item.Items, setting$, x#, y#, z#)
	...
	Select item\itemtemplate\name
		...
		Case "Планшет" ;Clipboard
			Select setting
				...
				Case "very fine"
					...
					ResetEntity(item\collider)
			End Select
		;Case "Cowbell" ;Что это тут вообще делает?
		;	Select setting
		;		Case "rough","coarse"
					...
					Select item\itemtemplate\name
						Case "Ключ-карта 1-го уровня" ;Level 1 Key Card
							Select SelectedDifficulty\otherFactors
								...
								Case NORMAL
									If Rand(5)=1 Then
										it2 = CreateItem("Mastercard", "misc", x, y, z) ;Mastercard
									Else
										it2 = CreateItem("Ключ-карта 2-го уровня", "key2", x, y, z) ;Level 2 Key Card
									EndIf
								Case HARD
		...
		Case "Mastercard"
			Select setting
				...
				Case "coarse"
					...
					it4 = CreateItem("Четвертак", "25ct", x, y, z) ;Quarter
					it5 = CreateItem("Четвертак", "25ct", x, y, z) ;Quarter
		...
		Case "9V-батарейка", "18V-батарейка", "Странная батарейка" ;9V Battery ;18V Battery ;Strange Battery
			...
			RemoveItem(item)
		Case "Глазные капли Зрение+", "Красные глазные капли Зрение+", "Глазные капли" ;ReVision Eyedrops ;RedVision Eyedrops ;Eyedrops
			Select setting
				Case "rough", "coarse"
					d.Decals = CreateDecal(0, x, 8 * RoomScale + 0.010, z, 90, Rand(360), 0)
					d\Size = 0.2 : EntityAlpha(d\obj, 0.8) : ScaleSprite(d\obj, d\Size, d\Size)
				Case "1:1"
					it2 = CreateItem("Красные глазные капли Зрение+", "eyedrops", x,y,z) ;RedVision Eyedrops
				Case "fine"
...
Function Use294() ;..!
	...
	If temp Then
		If MouseHit1 Then
			xtemp = Floor((MouseX()-x-190) / 35.5) ;xtemp = Floor((MouseX()-x-228) / 35.5)
...
;-----------------------------------------------------------	
	...
	Else ;playing a dispensing sound
		If Input294 <> "ВНЕ ЗОНЫ ДЕЙСТВИЯ" Then Input294 = "РАЗЛИВ..." ;OUT OF RANGE ;DISPENSING...
		
		If Not ChannelPlaying(PlayerRoom\SoundCHN) Then
			If Input294 <> "ВНЕ ЗОНЫ ДЕЙСТВИЯ" Then ;OUT OF RANGE
				HidePointer()
				Using294 = False
				MouseXSpeed() : MouseYSpeed() : MouseZSpeed() : mouse_x_speed_1#=0.0 : mouse_y_speed_1#=0.0
				Local e.Events
	...
	If I_427\Timer < 70*360
		If I_427\Using=True Then
			...
			If prevI427Timer < 70*60 And I_427\Timer => 70*60 Then
				Msg = "Вы чувствуете себя отдохнувшим и полным энергии." ;You feel refreshed and energetic.
				MsgTimer = 70*5
			ElseIf prevI427Timer < 70*180 And I_427\Timer => 70*180 Then
				Msg = "Вы чувствуете небольшие мышечные спазмы по всему телу." ;You feel gentle muscle spasms all over your body.
				MsgTimer = 70*5
			EndIf
		Else
			For i = 0 To 1
