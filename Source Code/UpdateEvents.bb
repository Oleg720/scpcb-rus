				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						ElseIf SelectedDifficulty\saveType = SAVEONSCREENS Then
							Msg = "Сохранение разрешено только у мониторов, разбросанных по всему комплексу." ;Saving is only permitted on clickable monitors scattered throughout the facility.
							MsgTimer = 70 * 8
						...
						If e\EventState3 < 170 Then
							...
							If e\EventState3 < 15 Then
								...
								Else
									...
									DropSpeed = 0
									e\EventState3 = 15
									Msg = "Возьмите бумагу на столе." ;Pick up the paper on the desk.
									MsgTimer=70*7
								EndIf
							...
							ElseIf e\EventState3 < 40
								If Inventory(0)<>Null Then
									Msg = "Нажмите "+KeyName(KEY_INV)+", чтобы открыть инвентарь.";Press ;to open the inventory.
									MsgTimer=70*7
									e\EventState3 = 40
									Exit
								EndIf
								...
								For i = 9 To 10
									...
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											...
											If KillTimer = 0 Then
												DeathMSG = "В дополнение к разложению, типичному всем жертвам SCP-106, на теле присутствуют повреждения, не наблюдавшиеся ранее: " ;In addition to the decomposed appearance typical of SCP-106's victims, the body exhibits injuries that have not been observed before: 
						...
						If EntityDistance(e\room\Objects[0], Collider)<1.5 Then
							...
							If EntityInView(e\room\Objects[0], Camera) Then
								...
								If MouseHit1 Then
									...
									Else
										Using294=False
										Msg = "Для использования этой машины, вам требуется вставить 50-ти центную монету." ;You need to insert a 50 cent coin in order to use this machine.
										MsgTimer = 70*5
									EndIf
								EndIf
							EndIf
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-FPSfactor =< 70*7.5 Then
								...
								For i = 0 To 2
									it2.Items = CreateItem("50 центная монета","50ct",1,1,1) ;50 Cent Coin
									it2\Picked = True
									it2\Dropped = -1
					...
					If e\EventState > 0.0 Then
						...
						Select Floor(e\EventState)
							...
							Case 500.0
								...
								MsgTimer = 70*5
							Case 700.0
								Msg = "Серьёзно, данный SCP не должен появляться." ;Seriously, this SCP should not spawn.
								MsgTimer = 70*5
							Case 800.0
								BlinkTimer = -2
								BlurTimer = 800
							...
							;DebugLog "WHERE IS IT?!"	
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>31*70 And e\EventState3-FPSfactor=<31*70
										...
										Msg="Вы разрываете левое запястье, и начинаете писать композицию своей кровью." ;You tear open your left wrist and start writing on the composition with your blood.
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>74*70 And e\EventState3-FPSfactor=<74*70
										...
										Msg="Вы широко открываете рану, черпая от туда кровь." ;You rip the wound wide open. Grabbing scoops of blood pouring out.
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
										DeathMSG = "Субъект D-9341 найден в луже крови рядом с SCP-012. Похоже, что субъект разорвал свои запястья и " ;Subject D-9341 found in a pool of blood next to SCP-012. Subject seems to have ripped open his wrists and written three extra 
										DeathMSG = DeathMSG + "написал три дополнительные строки в композиции перед смертью от потери крови." ;lines to the composition before dying of blood loss.
										Kill()
									EndIf
								...
								If EntityZ(Collider)>Min(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
									If EntityZ(Collider)<Max(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
										...
										If KillTimer < 0 And Bloodloss =>100 Then
											...
											DeathMSG = DeathMSG + "Субъект демонстрирует тяжёлое кровоизлияние вокруг глаз, рта и носа. " ;The subject exhibits heavy hemorrhaging of blood vessels around the eyes and inside the mouth and nose. 
						...
						Select e\EventState
							...
							Case 67
								If (Rand(150)=1) Then
									...
									DeathMSG = DeathMSG + "[REDACTED]. Тело субъекта D-9341 обнаружено внутри камеры. " ;[REDACTED]. The body of subject D-9341 was discovered inside the chamber. 
									DeathMSG = DeathMSG + "Субъект обнаруживает признаки травмы от удара тупым предметом, типичной для персонала, " ;The subject exhibits signs of blunt force trauma typical for personnel who have 
									DeathMSG = DeathMSG + "вошедшего в камеру во время выключения света." ;entered the chamber when the lights are off.
								...
								If EntityInView(fr\Door[i], Camera) Then
									...
									If MouseHit1 Then
										...
										Else
											...
											Msg = "Дверь не сдвигается с места." ;The door will not budge.
											MsgTimer = 70*5
										EndIf
									EndIf
								EndIf
							...
							If e\EventState > 70 * 3 Then
								Select setting
									Case "rough"
										...
										BlinkTimer = -10
										If e\SoundCHN <> 0 Then StopChannel e\SoundCHN
										DeathMSG = Chr(34)+"Сильно изуродованный труп был найден в отсеке выхода SCP-914. Труп был идентифицирован как субъект D-9341 после анализа ДНК. " ;A heavily mutilated corpse found inside the output booth of SCP-914. DNA testing identified the corpse as Class D Subject D-9341. 
										DeathMSG = DeathMSG + "Очевидно, что субъект был "+Chr(34)+"переработан"+Chr(34)+" SCP-914 на режиме "+Chr(34)+"Очень грубо"+Chr(34)+" но мы до сих пор не можем понять, как он " ;The subject had obviously been "+Chr(34)+"refined"+Chr(34)+" by SCP-914 on the "+Chr(34)+"Rough"+Chr(34)+" setting, but we are still confused as to how he 
										DeathMSG = DeathMSG + "оказался в отсеке входа и кто или что повернуло ключ."+Chr(34) ;ended up inside the intake booth and who or what wound the key.
									Case "coarse"
										BlinkTimer = -10
						...
						If e\EventState > (12 * 70) Then							
							...
							If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) < (160.0 * RoomScale) Then
								Select setting
									Case "coarse"
										Injuries = 4.0
										Msg = "Вы замечаете многочисленные маленькие разрезы по всему телу. Они сильно кровоточат." ;You notice countless small incisions all around your body. They are bleeding heavily.
										MsgTimer = 70*8
									Case "1:1"
					...
					Select e\EventState
						...
						Case 2
							...
							If (prevFrame>646.0) Then
								If (PlayerRoom = e\room) Then
									...
									Msg = "Что-то растёт вокруг вашего тела." ;Something is growing all around your body.
									MsgTimer = 70.0 * 3.0
								Else
									e\EventState3 = 70*30
								EndIf
						...
						Case 3
							...
							If (e\EventState2>250.0 And e\EventState2-FPSfactor <= 250.0) Then
								Select Rand(3)
									Case 1
										Msg = "Уши растут по всему телу." ;Ears are growing all over your body.
									Case 2
										Msg = "Ушные органы растут по всему телу." ;Ear-like organs are growing all over your body.
									Case 3
										Msg = "Уши растут по всему телу. Они ползают по вашей коже." ;Ears are growing all over your body. They are crawling on your skin.
								End Select
							...
							Else If (e\EventState2>600.0 And e\EventState2-FPSfactor <= 600.0)
								Select Rand(4)
									...
									Case 3
										Msg = "Уши растут внутри вашего тела." ;The ears are growing inside your body.
									Case 4
							...
							If (e\EventState2>70*15) Then
								DeathMSG = "Мёртвое тело было обнаружено в [УДАЛЕНО]. Субъект предположительно подвергся нападению со стороны SCP-1048-A и задохнулся от ушей. " ;A dead body covered in ears was found in [REDACTED]. Subject was presumably attacked by an instance of SCP-1048-A and suffocated to death by the ears. 
					...
					ElseIf e\EventState3 >= 3.0
						...
						Else
							...
							Else
								PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
								LightFlash = 5.0
								Msg = "Вы чувствуете, что что-то движется внутри ваших карманов, и внезапную боль в груди."
								MsgTimer = 70*5
							EndIf
							e\EventState2 = 0.0
						EndIf
