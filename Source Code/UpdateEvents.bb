				...
				If e\EventState = 0 Then
					If PlayerRoom = e\room Then
						...
						ElseIf SelectedDifficulty\saveType = SAVEONSCREENS Then
							Msg = "Сохранение разрешено только у мониторов, размещённых по всему комплексу." ;Saving is only permitted on clickable monitors scattered throughout the facility.
							MsgTimer = 70 * 8
						...
						If e\EventState3 < 170 Then
							...
							If e\EventState3 < 15 Then
								...
								Else
									...
									DropSpeed = 0
									e\EventState3 = 15
									Msg = "Возьмите бумагу со стола." ;Pick up the paper on the desk.
									MsgTimer=70*7
								EndIf
							...
							ElseIf e\EventState3 < 40
								If Inventory(0)<>Null Then
									Msg = "Нажмите "+KeyName(KEY_INV)+", чтобы открыть список предметов.";Press ;to open the inventory.
									MsgTimer=70*7
									e\EventState3 = 40
									Exit
								EndIf
								...
								Else
									...
									If (EntityDistance(Collider, e\room\NPC[3]\Collider)<1.5) Then
										If EntityInView(e\room\NPC[3]\obj, Camera) Then
											...
											If MouseHit1 Then
												SelectedItem = CreateItem("Документ об SCP-173", "paper", 0.0, 0.0, 0.0) ;Document SCP-173
						...
						If (CoffinDistance < 4.0) And (hasBatteryFor895) And (Not Wearing714) Then
							...
							If Sanity < -1000 Then
								If WearingNightVision > 1
									DeathMSG = Chr(34)+"Сотрудник Класса D смотрел на SCP-895 через цифровые очки ночного виденья, предположительно, улучшенные с помощью SCP-914. Вполне возможно, что субъект " ;Class D viewed SCP-895 through a pair of digital night vision goggles, presumably enhanced by SCP-914. It might be possible that the subject 
									DeathMSG = DeathMSG + "был способен частично противостоять меметическому воздействию с помощью этих очков. Они были взяты на хранение для дальнейшего исследования."+Chr(34) ;was able to resist the memetic effects partially through these goggles. The goggles have been stored for further study.
								Else
									DeathMSG = Chr(34)+"Сотрудник Класса D смотрел на SCP-895 через цифровые очки ночного видения, в результате чего погиб."+Chr(34) ;Class D viewed SCP-895 through a pair of digital night vision goggles, killing him.
								EndIf
					...
					If e\EventState2 = 1 Then ;in the second room
								...
								For i = 9 To 10
									...
									If dist<6.0 Then 
										If dist<100.0*RoomScale Then
											...
											If KillTimer = 0 Then
												DeathMSG = "В дополнение к разложению, типичному для всех жертв SCP-106, на теле присутствуют повреждения, не наблюдавшиеся ранее: " ;In addition to the decomposed appearance typical of SCP-106's victims, the body exhibits injuries that have not been observed before: 
												DeathMSG = DeathMSG + "массивный перелом черепа, три сломанных ребра, перелом плеча и множественные тяжелые раны." ;massive skull fracture, three broken ribs, fractured shoulder and multiple heavy lacerations.
						...
						If EntityY(Collider) < -1600*RoomScale Then
							...
							Else ;the player is not at the exit, must've fallen down
								...
								If KillTimer => 0 Then 
									PlaySound_Strict HorrorSFX(8)
									DeathMSG = "В дополнение к разложению, типичному всем жертвам SCP-106, субъект получил множественные тяжелые переломы обеих ног." ;In addition to the decomposed appearance typical of the victims of SCP-106, the subject seems to have suffered multiple heavy fractures to both of his legs.
					...
					Else
						...
						If BlinkTimer =-10 Then
							If e\EventState > 70*7.5 And e\EventState-FPSfactor =< 70*7.5 Then
								...
								For i = 0 To 1
									it2.Items = CreateItem("Четвертак","25ct",1,1,1) ;Quarter
									it2\Picked = True
									it2\Dropped = -1
								...
								If KillTimer => 0 Then 
									For i = 0 To 2
										If Distance(EntityX(Collider),EntityZ(Collider),EntityX(e\room\Objects[i],True),EntityZ(e\room\Objects[i],True)) < 250.0*RoomScale Then
											...
											CameraShake = 1.0
											Kill()
											DeathMSG = "Субъект D-9341 убит тесла-воротами в [УДАЛЕНО]." ;Subject D-9341 killed by the Tesla gate at [REDACTED].
										EndIf
									Next
								EndIf
				...
				If PlayerRoom = e\room Then
					...
					If e\room\grid = Null Then
						...
						If lastX=firstX And lastY=firstY Then
							RuntimeError("Туннели обслуживания не могут быть сгенерированы должным образом!") ;The maintenance tunnels could not be generated properly!
						EndIf
								...
								If e\room\grid\grid[ix+(iy*gridsz)]>0 Then
									...
									Select e\room\grid\grid[ix+(iy*gridsz)]
										...
										Case 7
											...
											it = CreateItem("Очки ночного виденья", "nvgoggles",e\room\x+ix*2.0-(Sin(EntityYaw(tempInt,True))*504.0*RoomScale)+(Cos(EntityYaw(tempInt,True))*16.0*RoomScale), 8.0+(80.0*RoomScale), e\room\z+iy*2.0+(Cos(EntityYaw(tempInt,True))*504.0*RoomScale)+(Sin(EntityYaw(tempInt,True))*16.0*RoomScale)) ;Night Vision Goggles
											EntityType (it\collider, HIT_ITEM)
									End Select
							...
							If (EntityDistance(Collider, e\room\Objects[2])<1.5) Then
								...
								If MouseHit1 Then
									If ItemAmount >= MaxItemAmount Then
										...
										MsgTimer = 70 * 5
									Else
										SelectedItem = CreateItem("Рисунок", "paper", 0.0, 0.0, 0.0) ;Drawing
										EntityType SelectedItem\collider,HIT_ITEM
					...
					If temp = True Then 
						...
						Select e\EventState 
							...
							Case 18
								...
								it.Items = CreateItem("Странная записка", "paper", TFormedX(), TFormedY(), TFormedZ()) ;Strange Note
								EntityType(it\collider, HIT_ITEM)
							Case 25
							...
							Case 30
								...
								If Inventory(i)<>Null Then RemoveItem(Inventory(i))
								Inventory(i) = CreateItem("Странная записка", "paper", 1,1,1) ;Strange Note
								HideEntity Inventory(i)\collider
								Inventory(i)\Picked = True
							Case 35
								For i = 0 To 3
				...
				If PlayerRoom=e\room Then
					...
					If e\EventState > 0.0 Then
						...
						Select Floor(e\EventState)
							Case 250.0
								Msg = "Привет!" ;Hey there!
								MsgTimer = 70*5
							Case 500.0
								Msg = "Если ты это видишь, то тебе нужно сообщить о данном баге." ;If you're seeing this, post a bug report.
								MsgTimer = 70*5
							Case 700.0
								Msg = "Серьёзно, данный SCP не должен появляться." ;Seriously, this SCP should not spawn. ; shouldn't --> should not why juan why --Mark
								MsgTimer = 70*5
							Case 800.0
								BlinkTimer = -2
								BlurTimer = 800
							Case 1200.0
								Msg = "Это событие - просто отстой." ;This event sucks.
								MsgTimer = 70*5
							Case 1400.0
								Msg = "Большое спасибо, Хуан." ;Thanks a lot, Juan.
								MsgTimer = 70*5
							Case 1500.0
								DeathMSG = "Твою же МАТЬ, Хуан. О чём ты думал?" ;God DAMMIT, Juan. What were you thinking?
								Kill()
						End Select
						...
						If PlayerRoom <> e\room Then
							;e\EventState = 2300.0
							If PlayerRoom\RoomTemplate\Name = "pocketdimension" Then DeathMSG = "Субъект D-9341 был пойман и убит SCP-106. Тело утилизировано путём сжигания." ;Subject D-9341 was captured and killed by SCP-106. Body was disposed of via incineration.
							Kill()
							Playable = True
						EndIf
						If KillTimer < 0 Then
							...
							;DebugLog "WHERE IS IT?!"	
								...
								If dist < 0.6 Then
									...
									ElseIf e\EventState3>13*70 And e\EventState3-FPSfactor=<13*70
										Msg="Вы начинаете вонзать ногти в свое запястье, чтобы пошла кровь." ;You start pushing your nails into your wrist, drawing blood.
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>31*70 And e\EventState3-FPSfactor=<31*70
										...
										Msg="Вы разрываете свое левое запястье и начинаете писать композицию своей кровью." ;You tear open your left wrist and start writing on the composition with your blood.
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>49*70 And e\EventState3-FPSfactor=<49*70
										Msg="Вы засовываете пальцы глубже в рану." ;You push your fingers deeper into the wound.
										MsgTimer = 8*70
										Injuries=Injuries+0.3
									...
									ElseIf e\EventState3>74*70 And e\EventState3-FPSfactor=<74*70
										...
										Msg="Вы широко раздираете рану, черпая оттуда кровь." ;You rip the wound wide open. Grabbing scoops of blood pouring out.
										MsgTimer = 7*70
									...
									ElseIf e\EventState3>85*70 And e\EventState3-FPSfactor=<85*70	
										DeathMSG = "Субъект D-9341 найден в луже крови рядом с SCP-012. Похоже, что субъект разодрал в кровь свои запястья и " ;Subject D-9341 found in a pool of blood next to SCP-012. Subject seems to have ripped open his wrists and written three extra 
										DeathMSG = DeathMSG + "написал три дополнительные строки в композиции, а затем умер от потери крови." ;lines to the composition before dying of blood loss.
										Kill()
									EndIf
								...
								If EntityZ(Collider)>Min(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
									If EntityZ(Collider)<Max(EntityZ(e\room\Objects[7],True),EntityZ(e\room\Objects[8],True)) Then
										...
										If KillTimer < 0 And Bloodloss =>100 Then
											DeathMSG = "Субъект D-9341 найден мёртвым внутри камеры содержания SCP-035. " ;Class D Subject D-9341 found dead inside SCP-035's containment chamber. 
											DeathMSG = DeathMSG + "Видны обширные кровоизлияния вокруг глаз субъекта, а также в полости рта и носа. " ;The subject exhibits heavy hemorrhaging of blood vessels around the eyes and inside the mouth and nose. 
											DeathMSG = DeathMSG + "Тело отправлено на экспертизу." ;Sent for autopsy.
										EndIf
									EndIf
								EndIf
							EndIf
					...
					Else
						...
						If e\EventState = 0 Then
							...
							If EntityDistance(e\room\Objects[11],Collider)<EntityDistance(e\room\Objects[12],Collider) Then
								it = CreateItem("Схема исследовательского сектора-02", "paper", EntityX(e\room\Objects[11],True),EntityY(e\room\Objects[11],True),EntityZ(e\room\Objects[11],True)) ;Research Sector-02 Scheme
								EntityType it\collider,HIT_ITEM
							Else
								it = CreateItem("Схема исследовательского сектора-02", "paper", EntityX(e\room\Objects[12],True),EntityY(e\room\Objects[12],True),EntityZ(e\room\Objects[12],True)) ;Research Sector-02 Scheme
								EntityType it\collider,HIT_ITEM
							EndIf
						ElseIf e\EventState > 0
				...
				If PlayerRoom = e\room Then
					...
					Else
						...
						Select e\EventState
							...
							Case 67
								If (Rand(150)=1) Then
									DeathMSG = "По-видимому, цикл SCP-205 вернулся к нормальному распорядку после аномалий, которые наблюдались во время " ;The SCP-205 cycle seems to have resumed its normal course after the anomalies observed during 
									DeathMSG = DeathMSG + "[УДАЛЕНО]. Тело субъекта D-9341 было найдено внутри камеры. " ;[REDACTED]. The body of subject D-9341 was discovered inside the chamber. 
									DeathMSG = DeathMSG + "Обнаружены признаки травмы от удара тупым предметом, что типично для всех, " ;The subject exhibits signs of blunt force trauma typical for personnel who have 
									DeathMSG = DeathMSG + "кто входит в камеру с выключенным светом." ;entered the chamber when the lights are off.
							...
							If e\EventState > 70 * 3 Then
								Select setting
									Case "rough"
										...
										BlinkTimer = -10
										If e\SoundCHN <> 0 Then StopChannel e\SoundCHN
										DeathMSG = Chr(34)+"В отсеке вывода SCP-914 был найден сильно изуродованный труп. После анализа ДНК труп был идентифицирован как субъект D-9341. " ;A heavily mutilated corpse found inside the output booth of SCP-914. DNA testing identified the corpse as Class D Subject D-9341. 
										DeathMSG = DeathMSG + "Очевидно, что субъект был "+Chr(34)+"переработан"+Chr(34)+" SCP-914 в режиме "+Chr(34)+"Очень грубо"+Chr(34)+" но мы до сих пор не можем понять, как он " ;The subject had obviously been "+Chr(34)+"refined"+Chr(34)+" by SCP-914 on the "+Chr(34)+"Rough"+Chr(34)+" setting, but we are still confused as to how he 
										DeathMSG = DeathMSG + "оказался в отсеке ввода и кто или что повернуло ключ."+Chr(34) ;ended up inside the intake booth and who or what wound the key.
									Case "coarse"
										BlinkTimer = -10
						...
						If e\EventState > (12 * 70) Then							
							...
							If Distance(EntityX(Collider), EntityZ(Collider), EntityX(e\room\Objects[2], True), EntityZ(e\room\Objects[2], True)) < (160.0 * RoomScale) Then
								Select setting
									Case "coarse"
										Injuries = 4.0
										Msg = "Вы замечаете многочисленные маленькие раны по всему телу. Они сильно кровоточат." ;You notice countless small incisions all around your body. They are bleeding heavily.
										MsgTimer = 70*8
									Case "1:1"
						...
						Else
							...
							For i = 0 To MaxItemAmount-1
								...
								If Rand(8)=1 Then
									...
									Select (e\EventState)
										Case 1
											itemName = "Потерянный ключ" ;Lost Key
										Case 2
											itemName = "Дисциплинарное слушание DH-S-4137-17092" ;Disciplinary Hearing DH-S-4137-17092
										Case 3
											itemName = "Монета" ;Coin
										Case 4
											itemName = "Билет в кино" ;"Movie Ticket
										Case 5
					...
					ElseIf e\EventState3 = 2.0
						...
						For itt.ItemTemplates = Each ItemTemplates
							If IsItemGoodFor1162(itt) And Rand(6)=1
								...
								e\EventState3 = 0.0
								If Injuries > 15
									DeathMSG = "Мёртвый субъект класса D был обнаружен внутри камеры содержания SCP-1162." ;A dead Class D subject was discovered within the containment chamber of SCP-1162.
									DeathMSG = DeathMSG + " Вскрытие показало, что его правое лёгкое отсутствовало, что предполагает" ; An autopsy revealed that his right lung was missing, which suggests
									DeathMSG = DeathMSG + " взаимодействие с SCP-1162." ;interaction with SCP-1162.
								...
								Else
									PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
									LightFlash = 5.0
									Msg = "Вы чувствуете внезапную боль в груди." ;You feel a sudden overwhelming pain in your chest.
									MsgTimer = 70*5
								EndIf
								Exit
							EndIf
					...
					ElseIf e\EventState3 >= 3.0
						...
						Else
							...
							Else
								PlaySound_Strict LoadTempSound("SFX\SCP\1162\BodyHorrorExchange"+Rand(1,4)+".ogg")
								LightFlash = 5.0
								Msg = "Вы чувствуете, как что-то движется внутри ваших карманов, и внезапную боль в груди." ;You notice something moving in your pockets and a sudden pain in your chest.
								MsgTimer = 70*5
							EndIf
							e\EventState2 = 0.0
						EndIf
						Select e\EventState
							Case 1
								it = CreateItem("Потерянный ключ","key",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True)) ;Lost Key
							Case 2
								it = CreateItem("Дисциплинарное слушание DH-S-4137-17092","oldpaper",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True)) ;Disciplinary Hearing DH-S-4137-17092
							Case 3
								it = CreateItem("Монета","coin",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True)) ;Coin
							Case 4
								it = CreateItem("Билет в кино","ticket",EntityX(pp,True),EntityY(pp,True),EntityZ(pp,True)) ;Movie Ticket
							Case 5
