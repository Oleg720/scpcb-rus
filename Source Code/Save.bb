...
Function LoadGame(file$)
	...
	If ReadInt(f) <> 113 Then RuntimeError("Не удалось загрузить игру, так как файл сохранения повреждён (ошибка 2.5).") ;Couldn't load the game, save file corrupted (error 2.5)
	...
	If ReadInt(f) <> 632 Then RuntimeError("Не удалось загрузить игру, так как файл сохранения повреждён (ошибка 1).") ;Couldn't load the game, save file corrupted (error 1)
...
;        EndIf
	...
	If ReadInt(f) <> 954 Then RuntimeError("Не удалось загрузить игру, так как файл сохранения повреждён (ошибка 2).") ;Couldn't load the game, save file may be corrupted (error 2)
