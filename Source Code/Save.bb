...
Function LoadGame(file$)
	...
	If (strtemp <> CompatibleNumber) Then RuntimeError("Файл сохранения v"+strtemp+" не совместим с SCP - Containment Breach "+VersionNumber+" (последняя совместимая версия: "+CompatibleNumber+").") ;The save files of v"+strtemp+" aren't compatible with SCP - Containment Breach "+VersionNumber+" (latest compatible version:
	...
	If ReadInt(f) <> 113 Then RuntimeError("Не удалось загрузить игру, т.к. файл сохранения повреждён (ошибка 2.5)") ;Couldn't load the game, save file corrupted (error 2.5)
	...
	If ReadInt(f) <> 632 Then RuntimeError("Не удалось загрузить игру, т.к. файл сохранения повреждён (ошибка 1)") ;Couldn't load the game, save file corrupted (error 1)
	...
	If ReadInt(f) <> 954 Then RuntimeError("Не удалось загрузить игру, т.к. файл сохранения повреждён (ошибка 2)") ;Couldn't load the game, save file may be corrupted (error 2
	...
	If ReadInt(f) <> 1845 Then RuntimeError("Не удалось загрузить игру, т.к. файл сохранения повреждён (ошибка 3)") ;Couldn't load the game, save file corrupted (error
...
Function LoadGameQuick(file$)
	...
	If ReadInt(f) <> 1845 Then RuntimeError("Не удалось загрузить игру, т.к. файл сохранения повреждён (ошибка 3)") ;Couldn't load the game, save file corrupted (error 3)
