...
Function StreamSound_Strict(file$,volume#=1.0,custommode=Mode)
	...
	If st\sfx = 0
		CreateConsoleMsg("Не удалось передать звук (возвращено 0): " + Chr(34) + file$ + Chr(34)) ;Failed to stream Sound (returned 0): 
		If ConsoleOpening
			ConsoleOpen = True
		EndIf
		Return 0
	...
	If st\chn = -1
		CreateConsoleMsg("Не удалось передать звук (возвращено -1): " + Chr(34) + file$ + Chr(34)) ;Failed to stream Sound (returned -1): 
		If ConsoleOpening
			ConsoleOpen = True
		EndIf
		Return -1
