...
Function LoadImage_Strict(file$)
	If FileType(file$)<>1 Then RuntimeError "Изображение " + Chr(34) + file$ + Chr(34) + " отсутствует" ;Image missing. 
	tmp = LoadImage(file$)
	Return tmp
		...
		For i = 0 To 31
			If snd\channels[i] <> 0 Then
				If Not ChannelPlaying(snd\channels[i]) Then
					If snd\internalHandle = 0 Then
						If FileType(snd\name) <> 1 Then
							CreateConsoleMsg("Звук " + Chr(34) + snd\name + Chr(34) + " не найден.") ;Sound not found.
							If ConsoleOpening
								ConsoleOpen = True
							EndIf
						Else
						...
						EndIf
						If snd\internalHandle = 0 Then
							CreateConsoleMsg("Не удалось загрузить звук: " + Chr(34) + snd\name + Chr(34)) ;Failed to load Sound: 
							If ConsoleOpening
								ConsoleOpen = True
							EndIf
						EndIf
...
Function StreamSound_Strict(file$,volume#=1.0,custommode=Mode)
	If FileType(file$)<>1
		CreateConsoleMsg("Звук " + Chr(34) + file$ + Chr(34) + " не найден.") ;Sound not found.
		If ConsoleOpening
			ConsoleOpen = True
		EndIf
		Return 0
	...
	If st\sfx = 0
		CreateConsoleMsg("Не удалось передать звук (возвращено значение 0): " + Chr(34) + file$ + Chr(34)) ;Failed to stream Sound (returned 0): 
		If ConsoleOpening
			ConsoleOpen = True
		EndIf
		Return 0
	...
	If st\chn = -1
		CreateConsoleMsg("Не удалось передать звук (возвращено значение -1): " + Chr(34) + file$ + Chr(34)) ;Failed to stream Sound (returned -1): 
		If ConsoleOpening
			ConsoleOpen = True
		EndIf
		Return -1
...
Function StopStream_Strict(streamHandle%)
	...
	If st = Null
		CreateConsoleMsg("Не удалось остановить передачу звука: Неизвестный поток") ;Failed to stop stream Sound: Unknown Stream
		Return
	EndIf
	If st\chn=0 Or st\chn=-1
		CreateConsoleMsg("Не удалось остановить передачу звука: Возвращено значение "+st\chn) ;Failed to stop stream Sound: Return value 
		Return
	EndIf
...
Function SetStreamVolume_Strict(streamHandle%,volume#)
	...
	If st = Null
		CreateConsoleMsg("Не удалось установить уровень громкости звука для потока: Неизвестный поток") ;Failed to set stream Sound volume: Unknown Stream
		Return
	EndIf
	If st\chn=0 Or st\chn=-1
		CreateConsoleMsg("Не удалось установить уровень громкости звука для потока: Возвращено значение "+st\chn) ;Failed to set stream Sound volume: Return value 
		Return
	EndIf
...
Function SetStreamPaused_Strict(streamHandle%,paused%)
	...
	If st = Null
		CreateConsoleMsg("Не удалось приостановить/возобновить передачу звука: Неизвестный поток") ;Failed to pause/unpause stream Sound: Unknown Stream
		Return
	EndIf
	If st\chn=0 Or st\chn=-1
		CreateConsoleMsg("Не удалось приостановить/возобновить передачу звука: Возвращено значение "+st\chn) ;Failed to pause/unpause stream Sound: Return value 
		Return
	EndIf
...
Function IsStreamPlaying_Strict(streamHandle%)
	...
	If st = Null
		CreateConsoleMsg("Не удалось найти поток звука: Неизвестный поток") ;Failed to find stream Sound: Unknown Stream
		Return
	EndIf
	If st\chn=0 Or st\chn=-1
		CreateConsoleMsg("Не удалось найти поток звука: Возвращено значение "+st\chn) ;Failed to find stream Sound: Return value
		Return
	EndIf
...
Function LoadTexture_Strict(File$,flags=1)
	If FileType(File$) <> 1 Then RuntimeError "Текстура " + File$ + " не найдена." ;Texture not found.
	tmp = LoadTexture(File$, flags+(256*(EnableVRam=True)))
	If tmp = 0 Then RuntimeError "Не удалось загрузить текстуру: " + File$ ;Failed to load Texture: 
	Return tmp 
End Function   

Function LoadBrush_Strict(file$,flags,u#=1.0,v#=1.0)
	If FileType(file$)<>1 Then RuntimeError "Текстура кисти " + file$ + " не найдена." ;Brush Texture not found.
	tmp = LoadBrush(file$, flags, u, v)
	If tmp = 0 Then RuntimeError "Не удалось загрузить текстуру кисти: " + file$ ;Failed to load Brush:
	Return tmp 
End Function 

Function LoadFont_Strict(file$="Arial Cyr", height=13, bold=0, italic=0, underline=0) ;Tahoma
	If FileType(file$)<>1 Then RuntimeError "Шрифт " + file$ + " не найден." ;Font not found.
