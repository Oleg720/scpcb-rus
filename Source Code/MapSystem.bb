...
Function LoadRMesh(file$,rt.RoomTemplates)
	...
	For i=0 To 3 ;reattempt up to 3 times
		...
		EndIf
	Next
	If f=0 Then RuntimeError "Ошибка чтения файла "+Chr(34)+file+Chr(34) ;
	...
	ElseIf isRMesh$="RoomMesh.HasTriggerBox"
		hasTriggerBox% = True
	Else
		RuntimeError Chr(34)+file+Chr(34)+" не RMESH ("+isRMesh+")" ;
	EndIf
			...
			If fr\grid[(ty*gridsize)+tx]=1 Then 
				...
				If tile_type > 0 Then 
					...
					If (ty Mod 3)=2 And itemPlaced[Floor(ty/3)]=False Then
						...
						it.Items = CreateItem("Журнал #"+Int(Floor(ty/3)+1), "paper", 0,0.5,0) ;Log #
...
;			EntityParent dp\cam,fr\Forest_Pivot
		...
		Case "room2sroom"
			...
			it = CreateItem("Документ об Ядерном устройстве", "paper", r\x + 2248.0 * RoomScale, r\y + 440.0 * RoomScale, r\z + 372.0 * RoomScale) ;Nuclear Device Document
			EntityParent(it\collider, r\obj)
		...
		Case "room2shaft"
			...
			it = CreateItem("Глазные капли РеЗрение", "eyedrops", r\x + 1930.0*RoomScale, r\y + 225.0 * RoomScale, r\z + 128.0*RoomScale) ;ReVision Eyedrops
			EntityParent(it\collider, r\obj)
		...
		Case "room2poffices"
			...
			EntityParent(it\collider, r\obj)
			it = CreateItem("Страница журнала", "paper", r\x + 912 * RoomScale, r\y + 176.0 * RoomScale, r\z - 160.0 * RoomScale) ;Journal Page
			EntityParent(it\collider, r\obj)
		...
		Case "room2nuke"
			...
			it = CreateItem("Документ об Ядерном устройстве", "paper", r\x - 768.0 * RoomScale, r\y + 1684.0 * RoomScale, r\z - 768.0 * RoomScale) ;Nuclear Device Document
			EntityParent(it\collider, r\obj)
...
;			PositionEntity(r\Objects[6], r\x - 848*RoomScale, 0.5, r\z-576*RoomScale, True)
		...
		Case "room2offices3"
			...
			For i = 0 To Rand(0,1)
				it = CreateItem("Глазные капли РеЗрение", "eyedrops", r\x - 1529.0*RoomScale, r\y + 563.0 * RoomScale, r\z - 572.0*RoomScale + i*0.05) ;ReVision Eyedrops
				EntityParent(it\collider, r\obj)				
			Next
		...
		Case "914"
			...
			it = CreateItem("Дополнение: 5/14 Протокол исследований", "paper", r\x +954.0 * RoomScale, r\y +228.0 * RoomScale, r\z + 127.0 * RoomScale) ;Addendum: 5/14 Test Log
			EntityParent(it\collider, r\obj)	
		...
		Case "173"
			...
			CreateItem("Ориентационная листовка класса D", "paper", r\x-(2914+1024)*RoomScale, 170.0*RoomScale, r\z+40*RoomScale) ;Class D Orientation Leaflet
		...
		Case "pocketdimension"
			...
			CreateItem("Разъеденный документ", "paper", EntityX(r\obj),0.5,EntityZ(r\obj)+3.5) ;Burnt Note
		...
		Else
			...
			If close = True Then
				If sc\Screen Then
					...
					If Sanity < (-1000) Then 
						DeathMSG = Chr(34)+"Что мы знаем, так это то, что он умер от сердечного приступа. Мне кажется, что это было вызвано SCP-895, хотя это влияние видеотехники не наблюдалось ранее. " ;What we know is that he died of cardiac arrest. My guess is that it was caused by SCP-895, although it has never been observed affecting video equipment from this far before. 
						DeathMSG = DeathMSG + "Необходимо дальнейшее тестирование, чтобы определить, расширяется ли "+Chr(34)+"Красная зона"+Chr(34)+" SCP-895"+Chr(34) ;Further testing is needed to determine whether SCP-895's "+Chr(34)+"Red Zone"+Chr(34)+" is increasing.
